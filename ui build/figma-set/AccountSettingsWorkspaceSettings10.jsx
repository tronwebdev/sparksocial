// figma node: 1044:7768 Account Settings (Workspace Settings)
export function AccountSettingsWorkspaceSettings10(_p = {}) {
  const props = _p;
  return (
    <div className={props.className} style={{
      width: 1728,
      height: 1117,
      overflow: "hidden",
      backgroundColor: "rgb(255,255,255)",
      position: "relative",
      ...props.style,
    }}>
      <div style={{
        position: "absolute",
        left: 15,
        top: 19,
        width: 1698,
        height: 1080,
        overflow: "hidden",
      }}>
        <div style={{
          position: "absolute",
          left: 0,
          top: 0,
          transform: "matrix(-1,0,0,1,1698,0)",
          transformOrigin: "0 0",
          width: 1698,
          height: 1080,
          borderRadius: 30,
          background: "linear-gradient(152.280deg, rgba(245,107,255,0.2) 2.93%, rgba(255,255,255,0) 64.58%), linear-gradient(209.569deg, rgba(108,232,255,0.3) 3.00%, rgba(255,255,255,0) 68.96%), linear-gradient(rgb(247,247,247),rgb(247,247,247))",
        }} />
        <span style={{
          position: "absolute",
          left: 138,
          top: 34,
          width: 97,
          height: 31,
          fontFamily: "\"CS Mollwish Regular Demo\", -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
          fontWeight: 400,
          fontSize: 27.493083953857422,
          whiteSpace: "nowrap",
          lineHeight: "100%",
          color: "rgb(12,12,12)",
        }}>Settings</span>
        <div style={{
          position: "absolute",
          left: 26,
          top: 30,
          width: 92,
          height: 36.392,
          overflow: "hidden",
        }}>
          <div style={{
            position: "absolute",
            left: 0,
            top: 0,
            width: 92,
            height: 36.392,
            overflow: "hidden",
          }}>
            <div style={{
              position: "absolute",
              left: 0,
              top: 0,
              width: 92,
              height: 36.392,
              borderRadius: 7.07198429107666,
              backdropFilter: "blur(22.532px)",
              boxShadow: "0 0 0 0.707px rgb(131,131,131)",
            }} />
          </div>
          <div style={{
            position: "absolute",
            left: 13.532,
            top: 7.43,
            width: 64.467,
            height: 21,
            overflow: "hidden",
          }}>
            <span style={{
              position: "absolute",
              left: 24.467,
              top: 0,
              width: 40,
              height: 21,
              fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
              fontWeight: 500,
              fontSize: 16.91486167907715,
              textAlign: "center",
              whiteSpace: "nowrap",
              lineHeight: 1.2690074443817139,
              color: "rgb(131,131,131)",
            }}>Back</span>
            <svg width={7.468} height={14.936} viewBox="0 0 7.468 14.936" fill="none" style={{
              position: "absolute",
              left: 0,
              top: 0,
              transform: "matrix(-1,0,0,1,7.468,3.695)",
              transformOrigin: "0 0",
              width: 7.468,
              height: 14.936,
              borderRadius: 0.7861213684082031,
              color: "rgb(131,131,131)",
            }}>
              <path d={"M 0.695 -0.695 C 0.311 -1.079 -0.311 -1.079 -0.695 -0.695 C -1.079 -0.311 -1.079 0.311 -0.695 0.695 L 0 0 L 0.695 -0.695 Z M -0.695 14.241 C -1.079 14.625 -1.079 15.247 -0.695 15.631 C -0.311 16.015 0.311 16.015 0.695 15.631 L 0 14.936 L -0.695 14.241 Z M 0 0 L -0.695 0.695 L 6.217 7.607 L 6.912 6.912 L 7.607 6.217 L 0.695 -0.695 L 0 0 Z M 6.912 8.024 L 6.217 7.329 L -0.695 14.241 L 0 14.936 L 0.695 15.631 L 7.607 8.719 L 6.912 8.024 Z M 6.912 6.912 L 6.217 7.607 C 6.141 7.53 6.141 7.406 6.217 7.329 L 6.912 8.024 L 7.607 8.719 C 8.298 8.028 8.298 6.908 7.607 6.217 L 6.912 6.912 Z"} fill="currentColor" fillRule="nonzero" />
            </svg>
          </div>
        </div>
        <div style={{
          position: "absolute",
          left: 377.001,
          top: 345,
          width: 994.773,
          height: 393,
          opacity: 0.4,
          overflow: "hidden",
        }}>
          <svg width={427.556} height={393} viewBox="0 0 427.556 393" fill="none" style={{
            position: "absolute",
            left: 567.217,
            top: 0,
            width: 427.556,
            height: 393,
            borderRadius: 53.74359130859375,
          }}>
            <path d={"M 74.014 150.687 L 331.452 18.213 C 352.478 7.393 378.106 11.588 394.586 28.547 L 400.61 34.747 C 416.708 51.313 420.419 76.312 409.83 96.84 L 287.036 334.874 C 271.51 364.97 232.356 373.38 205.841 352.314 L 65.173 240.554 C 34.614 216.275 39.31 168.546 74.014 150.687 Z"} fill="currentColor" fillRule="nonzero" />
          </svg>
          <svg width={427.556} height={393} viewBox="0 0 427.556 393" fill="none" style={{
            position: "absolute",
            left: 0,
            top: 0,
            transform: "matrix(-1,0,0,1,427.556,0)",
            transformOrigin: "0 0",
            width: 427.556,
            height: 393,
            borderRadius: 53.74359130859375,
          }}>
            <path d={"M 74.014 150.687 L 331.452 18.213 C 352.478 7.393 378.106 11.588 394.586 28.547 L 400.61 34.747 C 416.708 51.313 420.419 76.312 409.83 96.84 L 287.036 334.874 C 271.51 364.97 232.356 373.38 205.841 352.314 L 65.173 240.554 C 34.614 216.275 39.31 168.546 74.014 150.687 Z"} fill="currentColor" fillRule="nonzero" />
          </svg>
        </div>
      </div>
      <svg width={1646} height={1} viewBox="0 -0.500 1646 1" fill="none" style={{
        position: "absolute",
        left: 41,
        top: 117,
        width: 1646,
        height: 1,
        color: "rgba(131,131,131,0.25)",
      }}>
        <path d={"M 0 -0.5 C -0.276 -0.5 -0.5 -0.276 -0.5 0 C -0.5 0.276 -0.276 0.5 0 0.5 L 0 0 L 0 -0.5 Z M 1646 0.5 C 1646.276 0.5 1646.5 0.276 1646.5 0 C 1646.5 -0.276 1646.276 -0.5 1646 -0.5 L 1646 0 L 1646 0.5 Z M 0 0 L 0 0.5 L 1646 0.5 L 1646 0 L 1646 -0.5 L 0 -0.5 L 0 0 Z"} fill="currentColor" fillRule="nonzero" />
      </svg>
      <div style={{
        position: "absolute",
        left: 338,
        top: 143,
        width: 1350,
        height: 956,
        borderRadius: 20,
        backgroundColor: "rgb(255,255,255)",
      }} />
      <div style={{
        position: "absolute",
        left: 367,
        top: 351,
        width: 1297,
        height: 715,
        borderRadius: 20,
        backgroundColor: "rgba(131,131,131,0.05)",
      }} />
      <div style={{
        position: "absolute",
        left: 393,
        top: 454,
        width: 410,
        height: 136,
        overflow: "hidden",
      }}>
        <div style={{
          position: "absolute",
          left: 0,
          top: 0,
          width: 410,
          height: 136,
          borderRadius: 13.364087104797363,
          backgroundColor: "rgb(255,255,255)",
          boxShadow: "inset 0 0 0 1px rgba(12,12,12,0.4)",
        }} />
        <span style={{
          position: "absolute",
          left: 123,
          top: 25,
          width: 127,
          height: 26,
          fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
          fontWeight: 700,
          fontSize: 20,
          whiteSpace: "nowrap",
          lineHeight: "100%",
          color: "rgb(12,12,12)",
        }}>Media assets</span>
        <span style={{
          position: "absolute",
          left: 123,
          top: 56,
          width: 157,
          height: 58,
          fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
          fontWeight: 400,
          fontSize: 16,
          lineHeight: 1.809999942779541,
          color: "rgb(131,131,131)",
          whiteSpace: "pre-wrap",
          display: "inline-block",
        }}><span style={{ fontSize: 16 }}>{"Images, videos, fonts"}</span>{"\n"}<span style={{ fontWeight: 700, fontSize: 16 }}>{"1,284 files · 2.4 GB"}</span></span>
        <div style={{
          position: "absolute",
          left: 24,
          top: 30.5,
          width: 75,
          height: 75,
          overflow: "hidden",
        }}>
          <div style={{
            position: "absolute",
            left: 0,
            top: 0,
            width: 75,
            height: 75,
            borderRadius: 17.59407615661621,
            backgroundColor: "rgb(12,12,12)",
            boxShadow: "inset 0 0 0 1.173px rgba(12,12,12,0.4)",
          }} />
          <div style={{
            position: "absolute",
            left: 21.756,
            top: 21.758,
            width: 32.244,
            height: 32.243,
            overflow: "hidden",
          }}>
            <svg width={32.243} height={32.243} viewBox="0 0 32.243 32.243" fill="none" style={{
              position: "absolute",
              left: 0,
              top: 0,
              width: 32.243,
              height: 32.243,
              color: "rgb(255,255,255)",
            }}>
              <path d={"M 28.661 0 L 28.661 1 L 28.661 1 L 28.661 0 Z M 32.243 28.661 L 31.243 28.661 L 31.243 28.661 L 32.243 28.661 Z M 3.583 32.243 L 3.583 31.243 L 3.583 31.243 L 3.583 32.243 Z M 0 3.583 L 1 3.583 L 1 3.583 L 0 3.583 Z M 3.583 0 L 3.583 1 L 28.661 1 L 28.661 0 L 28.661 -1 L 3.583 -1 L 3.583 0 Z M 28.661 0 C 28.661 1 28.66 1 28.66 1 C 28.659 1 28.659 1 28.659 1 C 28.658 1 28.658 1 28.657 1 C 28.656 1 28.656 1 28.655 1 C 28.654 1 28.654 1 28.655 1 C 28.656 1 28.66 1 28.667 1 C 28.682 1.001 28.708 1.002 28.743 1.004 C 28.815 1.01 28.923 1.02 29.056 1.042 C 29.326 1.087 29.672 1.176 30.005 1.342 C 30.334 1.507 30.633 1.738 30.851 2.066 C 31.066 2.388 31.243 2.862 31.243 3.583 L 32.243 3.583 L 33.243 3.583 C 33.243 2.512 32.973 1.642 32.515 0.957 C 32.062 0.277 31.465 -0.163 30.899 -0.447 C 30.337 -0.728 29.786 -0.864 29.385 -0.93 C 29.182 -0.964 29.01 -0.982 28.886 -0.99 C 28.823 -0.995 28.772 -0.997 28.734 -0.999 C 28.715 -0.999 28.699 -1 28.687 -1 C 28.681 -1 28.675 -1 28.671 -1 C 28.669 -1 28.667 -1 28.665 -1 C 28.664 -1 28.663 -1 28.663 -1 C 28.662 -1 28.662 -1 28.662 -1 C 28.661 -1 28.661 -1 28.661 0 Z M 32.243 3.583 L 31.243 3.583 L 31.243 28.661 L 32.243 28.661 L 33.243 28.661 L 33.243 3.583 L 32.243 3.583 Z M 32.243 28.661 C 31.243 28.661 31.243 28.66 31.243 28.66 C 31.243 28.659 31.243 28.659 31.243 28.659 C 31.243 28.658 31.243 28.658 31.243 28.657 C 31.243 28.656 31.243 28.656 31.243 28.655 C 31.243 28.654 31.243 28.654 31.243 28.655 C 31.243 28.656 31.243 28.66 31.243 28.667 C 31.242 28.682 31.241 28.708 31.239 28.743 C 31.234 28.815 31.223 28.923 31.201 29.056 C 31.156 29.326 31.067 29.672 30.901 30.005 C 30.736 30.334 30.505 30.633 30.177 30.851 C 29.855 31.066 29.381 31.243 28.661 31.243 L 28.661 32.243 L 28.661 33.243 C 29.731 33.243 30.601 32.973 31.286 32.515 C 31.966 32.062 32.406 31.465 32.69 30.899 C 32.971 30.337 33.107 29.786 33.174 29.385 C 33.207 29.182 33.225 29.01 33.234 28.886 C 33.238 28.823 33.24 28.772 33.242 28.734 C 33.242 28.715 33.243 28.699 33.243 28.687 C 33.243 28.681 33.243 28.675 33.243 28.671 C 33.243 28.669 33.243 28.667 33.243 28.665 C 33.243 28.664 33.243 28.663 33.243 28.663 C 33.243 28.662 33.243 28.662 33.243 28.662 C 33.243 28.661 33.243 28.661 32.243 28.661 Z M 28.661 32.243 L 28.661 31.243 L 3.583 31.243 L 3.583 32.243 L 3.583 33.243 L 28.661 33.243 L 28.661 32.243 Z M 3.583 32.243 C 3.583 31.243 3.583 31.243 3.584 31.243 C 3.584 31.243 3.584 31.243 3.584 31.243 C 3.585 31.243 3.585 31.243 3.586 31.243 C 3.587 31.243 3.587 31.243 3.588 31.243 C 3.589 31.243 3.589 31.243 3.588 31.243 C 3.587 31.243 3.583 31.243 3.576 31.243 C 3.561 31.242 3.535 31.241 3.5 31.239 C 3.429 31.234 3.32 31.223 3.187 31.201 C 2.917 31.156 2.572 31.067 2.238 30.901 C 1.909 30.736 1.611 30.505 1.392 30.177 C 1.177 29.855 1 29.381 1 28.661 L 0 28.661 L -1 28.661 C -1 29.731 -0.729 30.601 -0.272 31.286 C 0.181 31.966 0.778 32.406 1.344 32.69 C 1.907 32.971 2.457 33.107 2.858 33.174 C 3.061 33.207 3.233 33.225 3.357 33.234 C 3.42 33.238 3.471 33.24 3.509 33.242 C 3.528 33.242 3.544 33.243 3.556 33.243 C 3.562 33.243 3.568 33.243 3.572 33.243 C 3.574 33.243 3.576 33.243 3.578 33.243 C 3.579 33.243 3.58 33.243 3.58 33.243 C 3.581 33.243 3.581 33.243 3.582 33.243 C 3.582 33.243 3.583 33.243 3.583 32.243 Z M 0 28.661 L 1 28.661 L 1 3.583 L 0 3.583 L -1 3.583 L -1 28.661 L 0 28.661 Z M 0 3.583 C 1 3.583 1 3.583 1 3.584 C 1 3.584 1 3.584 1 3.584 C 1 3.585 1 3.585 1 3.586 C 1 3.587 1 3.587 1 3.588 C 1 3.589 1 3.589 1 3.588 C 1 3.587 1 3.583 1 3.576 C 1.001 3.561 1.002 3.535 1.004 3.5 C 1.01 3.429 1.02 3.32 1.042 3.187 C 1.087 2.917 1.176 2.572 1.342 2.238 C 1.507 1.909 1.738 1.611 2.066 1.392 C 2.388 1.177 2.862 1 3.583 1 L 3.583 0 L 3.583 -1 C 2.512 -1 1.642 -0.729 0.957 -0.272 C 0.277 0.181 -0.163 0.778 -0.447 1.344 C -0.728 1.907 -0.864 2.457 -0.93 2.858 C -0.964 3.061 -0.982 3.233 -0.99 3.357 C -0.995 3.42 -0.997 3.471 -0.999 3.509 C -0.999 3.528 -1 3.544 -1 3.556 C -1 3.562 -1 3.568 -1 3.572 C -1 3.574 -1 3.576 -1 3.578 C -1 3.579 -1 3.58 -1 3.58 C -1 3.581 -1 3.581 -1 3.582 C -1 3.582 -1 3.583 0 3.583 Z"} fill="currentColor" fillRule="nonzero" />
            </svg>
            <svg width={7.165} height={7.165} viewBox="0 0 7.165 7.165" fill="none" style={{
              position: "absolute",
              left: 7.166,
              top: 7.163,
              width: 7.165,
              height: 7.165,
              color: "rgb(255,255,255)",
            }}>
              <path d={"M 0 3.583 L 1 3.583 L 1 3.583 L 0 3.583 Z M 3.583 7.165 L 3.583 8.165 L 3.583 7.165 Z M 7.165 3.583 L 6.165 3.583 L 6.165 3.583 L 7.165 3.583 Z M 0 3.583 L -1 3.583 C -1 4.184 -0.881 4.78 -0.651 5.336 L 0.273 4.954 L 1.197 4.571 C 1.067 4.258 1 3.922 1 3.583 L 0 3.583 Z M 0.273 4.954 L -0.651 5.336 C -0.421 5.892 -0.083 6.397 0.342 6.823 L 1.049 6.116 L 1.756 5.409 C 1.517 5.169 1.326 4.884 1.197 4.571 L 0.273 4.954 Z M 1.049 6.116 L 0.342 6.823 C 0.768 7.248 1.273 7.586 1.829 7.816 L 2.212 6.892 L 2.594 5.969 C 2.281 5.839 1.996 5.649 1.756 5.409 L 1.049 6.116 Z M 2.212 6.892 L 1.829 7.816 C 2.385 8.047 2.981 8.165 3.583 8.165 L 3.583 7.165 L 3.583 6.165 C 3.243 6.165 2.908 6.098 2.594 5.969 L 2.212 6.892 Z M 3.583 7.165 L 3.583 8.165 C 4.184 8.165 4.78 8.047 5.336 7.816 L 4.954 6.892 L 4.571 5.969 C 4.258 6.098 3.922 6.165 3.583 6.165 L 3.583 7.165 Z M 4.954 6.892 L 5.336 7.816 C 5.892 7.586 6.397 7.248 6.823 6.823 L 6.116 6.116 L 5.409 5.409 C 5.169 5.649 4.884 5.839 4.571 5.969 L 4.954 6.892 Z M 6.116 6.116 L 6.823 6.823 C 7.248 6.397 7.586 5.892 7.816 5.336 L 6.892 4.954 L 5.969 4.571 C 5.839 4.884 5.649 5.169 5.409 5.409 L 6.116 6.116 Z M 6.892 4.954 L 7.816 5.336 C 8.047 4.78 8.165 4.184 8.165 3.583 L 7.165 3.583 L 6.165 3.583 C 6.165 3.922 6.098 4.258 5.969 4.571 L 6.892 4.954 Z M 7.165 3.583 L 8.165 3.583 C 8.165 2.981 8.047 2.385 7.816 1.829 L 6.892 2.212 L 5.969 2.594 C 6.098 2.908 6.165 3.243 6.165 3.583 L 7.165 3.583 Z M 6.892 2.212 L 7.816 1.829 C 7.586 1.273 7.248 0.768 6.823 0.342 L 6.116 1.049 L 5.409 1.756 C 5.649 1.996 5.839 2.281 5.969 2.594 L 6.892 2.212 Z M 6.116 1.049 L 6.823 0.342 C 6.397 -0.083 5.892 -0.421 5.336 -0.651 L 4.954 0.273 L 4.571 1.197 C 4.884 1.326 5.169 1.517 5.409 1.756 L 6.116 1.049 Z M 4.954 0.273 L 5.336 -0.651 C 4.78 -0.881 4.184 -1 3.583 -1 L 3.583 0 L 3.583 1 C 3.922 1 4.258 1.067 4.571 1.197 L 4.954 0.273 Z M 3.583 0 L 3.583 -1 C 2.981 -1 2.385 -0.881 1.829 -0.651 L 2.212 0.273 L 2.594 1.197 C 2.908 1.067 3.243 1 3.583 1 L 3.583 0 Z M 2.212 0.273 L 1.829 -0.651 C 1.273 -0.421 0.768 -0.083 0.342 0.342 L 1.049 1.049 L 1.756 1.756 C 1.996 1.517 2.281 1.326 2.594 1.197 L 2.212 0.273 Z M 1.049 1.049 L 0.342 0.342 C -0.083 0.768 -0.421 1.273 -0.651 1.829 L 0.273 2.212 L 1.197 2.594 C 1.326 2.281 1.517 1.996 1.756 1.756 L 1.049 1.049 Z M 0.273 2.212 L -0.651 1.829 C -0.881 2.385 -1 2.981 -1 3.583 L 0 3.583 L 1 3.583 C 1 3.243 1.067 2.908 1.197 2.594 L 0.273 2.212 Z"} fill="currentColor" fillRule="nonzero" />
            </svg>
            <svg width={26.869} height={17.325} viewBox="0 0 26.869 17.325" fill="none" style={{
              position: "absolute",
              left: 5.375,
              top: 14.912,
              width: 26.869,
              height: 17.325,
              color: "rgb(255,255,255)",
            }}>
              <path d={"M 26.162 7.284 C 26.553 7.674 27.186 7.674 27.576 7.284 C 27.967 6.893 27.967 6.26 27.576 5.87 L 26.869 6.577 L 26.162 7.284 Z M 21.341 1.049 L 22.048 0.342 L 22.048 0.342 L 21.341 1.049 Z M 16.276 1.049 L 15.569 0.342 L 15.569 0.342 L 16.276 1.049 Z M -0.707 16.617 C -1.098 17.008 -1.098 17.641 -0.707 18.032 C -0.317 18.422 0.317 18.422 0.707 18.032 L 0 17.325 L -0.707 16.617 Z M 26.869 6.577 L 27.576 5.87 L 22.048 0.342 L 21.341 1.049 L 20.634 1.756 L 26.162 7.284 L 26.869 6.577 Z M 21.341 1.049 L 22.048 0.342 C 21.189 -0.517 20.024 -1 18.808 -1 L 18.808 0 L 18.808 1 C 19.493 1 20.15 1.272 20.634 1.756 L 21.341 1.049 Z M 18.808 0 L 18.808 -1 C 17.593 -1 16.428 -0.517 15.569 0.342 L 16.276 1.049 L 16.983 1.756 C 17.467 1.272 18.124 1 18.808 1 L 18.808 0 Z M 16.276 1.049 L 15.569 0.342 L -0.707 16.617 L 0 17.325 L 0.707 18.032 L 16.983 1.756 L 16.276 1.049 Z"} fill="currentColor" fillRule="nonzero" />
            </svg>
          </div>
        </div>
        <div style={{
          position: "absolute",
          left: 376,
          top: 11,
          width: 24,
          height: 24,
          overflow: "hidden",
        }}>
          <div style={{
            position: "absolute",
            left: 0,
            top: 0,
            width: 24,
            height: 24,
            borderRadius: 5.6301045417785645,
            backgroundColor: "rgb(12,12,12)",
          }} />
          <svg width={9.745} height={9.750} viewBox="0 0 9.745 9.750" fill="none" style={{
            position: "absolute",
            left: 7,
            top: 7,
            width: 9.745,
            height: 9.75,
            color: "rgb(255,255,255)",
          }}>
            <path d={"M 9.422 0.408 C 9.532 0.529 9.617 0.671 9.672 0.824 C 9.727 0.978 9.752 1.141 9.744 1.305 C 9.736 1.468 9.695 1.628 9.626 1.776 C 9.556 1.923 9.457 2.056 9.336 2.166 C 8.212 3.185 7.451 4.026 6.824 5.004 C 6.191 5.993 5.66 7.173 5.053 8.916 C 4.983 9.116 4.863 9.296 4.705 9.437 C 4.546 9.577 4.354 9.675 4.147 9.721 C 3.939 9.767 3.724 9.759 3.521 9.698 C 3.317 9.637 3.133 9.524 2.985 9.372 L 0.344 6.649 C 0.117 6.412 -0.006 6.095 0 5.766 C 0.007 5.438 0.142 5.126 0.378 4.898 C 0.614 4.669 0.93 4.543 1.258 4.547 C 1.586 4.551 1.9 4.684 2.13 4.918 L 3.408 6.236 C 3.813 5.266 4.237 4.432 4.73 3.663 C 5.516 2.436 6.446 1.429 7.665 0.323 C 7.786 0.214 7.927 0.128 8.081 0.073 C 8.235 0.018 8.398 -0.006 8.561 0.001 C 8.724 0.009 8.884 0.049 9.032 0.119 C 9.18 0.189 9.313 0.287 9.422 0.408 Z"} fill="currentColor" fillRule="evenodd" />
          </svg>
        </div>
      </div>
      <div style={{
        position: "absolute",
        left: 393,
        top: 611,
        width: 410,
        height: 136,
        overflow: "hidden",
      }}>
        <div style={{
          position: "absolute",
          left: 0,
          top: 0,
          width: 410,
          height: 136,
          borderRadius: 13.364087104797363,
          backgroundColor: "rgb(255,255,255)",
          boxShadow: "inset 0 0 0 1px rgba(12,12,12,0.4)",
        }} />
        <span style={{
          position: "absolute",
          left: 123,
          top: 25,
          width: 191,
          height: 26,
          fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
          fontWeight: 700,
          fontSize: 20,
          whiteSpace: "nowrap",
          lineHeight: "100%",
          color: "rgb(12,12,12)",
        }}>Workspace settings</span>
        <span style={{
          position: "absolute",
          left: 123,
          top: 56,
          width: 194,
          height: 58,
          fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
          fontWeight: 400,
          fontSize: 16,
          lineHeight: 1.809999942779541,
          color: "rgb(131,131,131)",
          whiteSpace: "pre-wrap",
          display: "inline-block",
        }}>{"Roles, integrations, theme"}{"\n"}<span style={{ fontWeight: 700, fontSize: 16 }}>{"32 CONFIGS"}</span></span>
        <div style={{
          position: "absolute",
          left: 24,
          top: 30.5,
          width: 75,
          height: 75,
          overflow: "hidden",
        }}>
          <div style={{
            position: "absolute",
            left: 0,
            top: 0,
            width: 75,
            height: 75,
            borderRadius: 17.59407615661621,
            backgroundColor: "rgb(12,12,12)",
            boxShadow: "inset 0 0 0 1.173px rgba(12,12,12,0.4)",
          }} />
          <div style={{
            position: "absolute",
            left: 18.757,
            top: 18.755,
            width: 38.244,
            height: 38.244,
            overflow: "hidden",
          }}>
            <svg width={38.244} height={38.244} viewBox="0 0 38.244 38.244" fill="none" style={{
              position: "absolute",
              left: 0,
              top: 0.001,
              width: 38.244,
              height: 38.244,
              color: "rgb(255,255,255)",
            }}>
              <path d={"M 37.03 16.722 L 37.622 15.916 L 37.622 15.916 L 37.03 16.722 Z M 37.03 21.522 L 37.622 22.327 L 37.622 22.327 L 37.03 21.522 Z M 33.49 30.08 L 34.479 29.929 L 34.479 29.929 L 33.49 30.08 Z M 30.08 33.49 L 29.929 34.479 L 29.929 34.479 L 30.08 33.49 Z M 21.522 37.03 L 22.327 37.622 L 22.327 37.622 L 21.522 37.03 Z M 16.722 37.03 L 15.916 37.622 L 15.916 37.622 L 16.722 37.03 Z M 8.164 33.49 L 8.315 34.479 L 8.315 34.479 L 8.164 33.49 Z M 4.753 30.08 L 5.742 30.231 L 5.742 30.231 L 4.753 30.08 Z M 1.214 21.522 L 0.622 22.327 L 0.622 22.327 L 1.214 21.522 Z M 1.214 16.722 L 0.622 15.916 L 0.622 15.916 L 1.214 16.722 Z M 4.753 8.164 L 5.742 8.013 L 5.742 8.013 L 4.753 8.164 Z M 8.164 4.753 L 8.013 5.742 L 8.013 5.742 L 8.164 4.753 Z M 16.722 1.214 L 15.916 0.622 L 15.916 0.622 L 16.722 1.214 Z M 21.522 1.214 L 22.327 0.622 L 22.327 0.622 L 21.522 1.214 Z M 30.08 4.753 L 30.231 5.742 L 30.231 5.742 L 30.08 4.753 Z M 33.49 8.164 L 34.479 8.315 L 34.479 8.315 L 33.49 8.164 Z M 37.03 16.722 L 36.437 17.528 C 37.513 18.318 37.513 19.925 36.437 20.716 L 37.03 21.522 L 37.622 22.327 C 39.784 20.738 39.784 17.506 37.622 15.916 L 37.03 16.722 Z M 37.03 21.522 L 36.437 20.716 C 33.458 22.906 31.944 26.578 32.502 30.231 L 33.49 30.08 L 34.479 29.929 C 34.033 27.009 35.244 24.075 37.622 22.327 L 37.03 21.522 Z M 33.49 30.08 L 32.502 30.231 C 32.704 31.557 31.557 32.704 30.231 32.502 L 30.08 33.49 L 29.929 34.479 C 32.589 34.885 34.885 32.589 34.479 29.929 L 33.49 30.08 Z M 30.08 33.49 L 30.231 32.502 C 26.578 31.944 22.906 33.458 20.716 36.437 L 21.522 37.03 L 22.327 37.622 C 24.075 35.244 27.009 34.033 29.929 34.479 L 30.08 33.49 Z M 21.522 37.03 L 20.716 36.437 C 19.925 37.513 18.318 37.513 17.528 36.437 L 16.722 37.03 L 15.916 37.622 C 17.506 39.784 20.738 39.784 22.327 37.622 L 21.522 37.03 Z M 16.722 37.03 L 17.528 36.437 C 15.338 33.458 11.666 31.944 8.013 32.502 L 8.164 33.49 L 8.315 34.479 C 11.235 34.033 14.169 35.244 15.916 37.622 L 16.722 37.03 Z M 8.164 33.49 L 8.013 32.502 C 6.687 32.704 5.539 31.556 5.742 30.231 L 4.753 30.08 L 3.765 29.929 C 3.359 32.589 5.655 34.885 8.315 34.479 L 8.164 33.49 Z M 4.753 30.08 L 5.742 30.231 C 6.3 26.578 4.786 22.906 1.807 20.716 L 1.214 21.522 L 0.622 22.327 C 3 24.075 4.211 27.009 3.765 29.929 L 4.753 30.08 Z M 1.214 21.522 L 1.807 20.716 C 0.731 19.925 0.731 18.318 1.807 17.528 L 1.214 16.722 L 0.622 15.916 C -1.541 17.506 -1.541 20.738 0.622 22.327 L 1.214 21.522 Z M 1.214 16.722 L 1.807 17.528 C 4.786 15.338 6.3 11.666 5.742 8.013 L 4.753 8.164 L 3.765 8.315 C 4.211 11.235 3 14.169 0.622 15.916 L 1.214 16.722 Z M 4.753 8.164 L 5.742 8.013 C 5.539 6.687 6.687 5.539 8.013 5.742 L 8.164 4.753 L 8.315 3.765 C 5.655 3.359 3.359 5.655 3.765 8.315 L 4.753 8.164 Z M 8.164 4.753 L 8.013 5.742 C 11.666 6.3 15.338 4.786 17.528 1.807 L 16.722 1.214 L 15.916 0.622 C 14.169 3 11.235 4.211 8.315 3.765 L 8.164 4.753 Z M 16.722 1.214 L 17.528 1.807 C 18.318 0.731 19.925 0.731 20.716 1.807 L 21.522 1.214 L 22.327 0.622 C 20.738 -1.541 17.506 -1.541 15.916 0.622 L 16.722 1.214 Z M 21.522 1.214 L 20.716 1.807 C 22.906 4.786 26.578 6.3 30.231 5.742 L 30.08 4.753 L 29.929 3.765 C 27.009 4.211 24.075 3 22.327 0.622 L 21.522 1.214 Z M 30.08 4.753 L 30.231 5.742 C 31.556 5.539 32.704 6.687 32.502 8.013 L 33.49 8.164 L 34.479 8.315 C 34.885 5.655 32.589 3.359 29.929 3.765 L 30.08 4.753 Z M 33.49 8.164 L 32.502 8.013 C 31.944 11.666 33.458 15.338 36.437 17.528 L 37.03 16.722 L 37.622 15.916 C 35.244 14.169 34.033 11.235 34.479 8.315 L 33.49 8.164 Z"} fill="currentColor" fillRule="nonzero" />
            </svg>
            <svg width={15.649} height={15.649} viewBox="0 0 15.649 15.649" fill="none" style={{
              position: "absolute",
              left: 11.296,
              top: 11.297,
              width: 15.649,
              height: 15.649,
              color: "rgb(255,255,255)",
            }}>
              <path d={"M 7.825 15.649 L 7.825 16.649 C 10.513 16.649 12.754 15.89 14.322 14.322 C 15.89 12.754 16.649 10.512 16.649 7.825 L 15.649 7.825 L 14.649 7.825 C 14.649 10.144 14 11.815 12.908 12.908 C 11.816 14 10.144 14.649 7.825 14.649 L 7.825 15.649 Z M 15.649 7.825 L 16.649 7.825 C 16.649 5.137 15.89 2.895 14.322 1.327 C 12.754 -0.241 10.513 -1 7.825 -1 L 7.825 0 L 7.825 1 C 10.144 1 11.816 1.649 12.908 2.741 C 14 3.834 14.649 5.505 14.649 7.825 L 15.649 7.825 Z M 7.825 0 L 7.825 -1 C 5.137 -1 2.896 -0.241 1.327 1.327 C -0.241 2.895 -1 5.137 -1 7.825 L 0 7.825 L 1 7.825 C 1 5.505 1.649 3.834 2.742 2.741 C 3.834 1.649 5.505 1 7.825 1 L 7.825 0 Z M 0 7.825 L -1 7.825 C -1 10.512 -0.241 12.754 1.327 14.322 C 2.896 15.89 5.137 16.649 7.825 16.649 L 7.825 15.649 L 7.825 14.649 C 5.505 14.649 3.834 14 2.742 12.908 C 1.649 11.815 1 10.144 1 7.825 L 0 7.825 Z"} fill="currentColor" fillRule="nonzero" />
            </svg>
          </div>
        </div>
        <div style={{
          position: "absolute",
          left: 376,
          top: 11,
          width: 24,
          height: 24,
          overflow: "hidden",
        }}>
          <div style={{
            position: "absolute",
            left: 0,
            top: 0,
            width: 24,
            height: 24,
            borderRadius: 5.6301045417785645,
            backgroundColor: "rgb(12,12,12)",
          }} />
          <svg width={9.745} height={9.750} viewBox="0 0 9.745 9.750" fill="none" style={{
            position: "absolute",
            left: 7,
            top: 7,
            width: 9.745,
            height: 9.75,
            color: "rgb(255,255,255)",
          }}>
            <path d={"M 9.422 0.408 C 9.532 0.529 9.617 0.671 9.672 0.824 C 9.727 0.978 9.752 1.141 9.744 1.305 C 9.736 1.468 9.695 1.628 9.626 1.776 C 9.556 1.923 9.457 2.056 9.336 2.166 C 8.212 3.185 7.451 4.026 6.824 5.004 C 6.191 5.993 5.66 7.173 5.053 8.916 C 4.983 9.116 4.863 9.296 4.705 9.437 C 4.546 9.577 4.354 9.675 4.147 9.721 C 3.939 9.767 3.724 9.759 3.521 9.698 C 3.317 9.637 3.133 9.524 2.985 9.372 L 0.344 6.649 C 0.117 6.412 -0.006 6.095 0 5.766 C 0.007 5.438 0.142 5.126 0.378 4.898 C 0.614 4.669 0.93 4.543 1.258 4.547 C 1.586 4.551 1.9 4.684 2.13 4.918 L 3.408 6.236 C 3.813 5.266 4.237 4.432 4.73 3.663 C 5.516 2.436 6.446 1.429 7.665 0.323 C 7.786 0.214 7.927 0.128 8.081 0.073 C 8.235 0.018 8.398 -0.006 8.561 0.001 C 8.724 0.009 8.884 0.049 9.032 0.119 C 9.18 0.189 9.313 0.287 9.422 0.408 Z"} fill="currentColor" fillRule="evenodd" />
          </svg>
        </div>
      </div>
      <div style={{
        position: "absolute",
        left: 818,
        top: 454,
        width: 410,
        height: 136,
        overflow: "hidden",
      }}>
        <div style={{
          position: "absolute",
          left: 0,
          top: 0,
          width: 410,
          height: 136,
          borderRadius: 13.364087104797363,
          backgroundColor: "rgb(255,255,255)",
          boxShadow: "inset 0 0 0 1px rgba(12,12,12,0.4)",
        }} />
        <span style={{
          position: "absolute",
          left: 123,
          top: 25,
          width: 99,
          height: 26,
          fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
          fontWeight: 700,
          fontSize: 20,
          whiteSpace: "nowrap",
          lineHeight: "100%",
          color: "rgb(12,12,12)",
        }}>Templates</span>
        <span style={{
          position: "absolute",
          left: 123,
          top: 56,
          width: 202,
          height: 58,
          fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
          fontWeight: 400,
          fontSize: 16,
          lineHeight: 1.809999942779541,
          color: "rgb(131,131,131)",
          whiteSpace: "pre-wrap",
          display: "inline-block",
        }}>{"Pages, emails, components"}{"\n"}<span style={{ fontWeight: 700 }}>{"47 templates"}</span></span>
        <div style={{
          position: "absolute",
          left: 24,
          top: 30.5,
          width: 75,
          height: 75,
          overflow: "hidden",
        }}>
          <div style={{
            position: "absolute",
            left: 0,
            top: 0,
            width: 75,
            height: 75,
            borderRadius: 17.59407615661621,
            backgroundColor: "rgb(12,12,12)",
            boxShadow: "inset 0 0 0 1.173px rgba(12,12,12,0.4)",
          }} />
          <div style={{
            position: "absolute",
            left: 22.756,
            top: 22.757,
            width: 30.244,
            height: 30.244,
            overflow: "hidden",
          }}>
            <svg width={30.244} height={30.244} viewBox="0 0 30.244 30.244" fill="none" style={{
              position: "absolute",
              left: 0,
              top: -0.001,
              width: 30.244,
              height: 30.244,
              color: "rgb(255,255,255)",
            }}>
              <path d={"M 0 2.16 L -1 2.16 L -1 28.084 L 0 28.084 L 1 28.084 L 1 2.16 L 0 2.16 Z M 0 28.084 L -1 28.084 C -1 29.829 0.415 31.244 2.16 31.244 L 2.16 30.244 L 2.16 29.244 C 1.519 29.244 1 28.725 1 28.084 L 0 28.084 Z M 2.16 30.244 L 2.16 31.244 L 28.084 31.244 L 28.084 30.244 L 28.084 29.244 L 2.16 29.244 L 2.16 30.244 Z M 28.084 30.244 L 28.084 31.244 C 29.829 31.244 31.244 29.829 31.244 28.084 L 30.244 28.084 L 29.244 28.084 C 29.244 28.725 28.725 29.244 28.084 29.244 L 28.084 30.244 Z M 30.244 28.084 L 31.244 28.084 L 31.244 2.16 L 30.244 2.16 L 29.244 2.16 L 29.244 28.084 L 30.244 28.084 Z M 30.244 2.16 L 31.244 2.16 C 31.244 0.415 29.829 -1 28.084 -1 L 28.084 0 L 28.084 1 C 28.725 1 29.244 1.519 29.244 2.16 L 30.244 2.16 Z M 28.084 0 L 28.084 -1 L 2.16 -1 L 2.16 0 L 2.16 1 L 28.084 1 L 28.084 0 Z M 2.16 0 L 2.16 -1 C 0.415 -1 -1 0.415 -1 2.16 L 0 2.16 L 1 2.16 C 1 1.519 1.519 1 2.16 1 L 2.16 0 Z"} fill="currentColor" fillRule="nonzero" />
            </svg>
            <svg width={2} height={30.244} viewBox="-1 0 2 30.244" fill="none" style={{
              position: "absolute",
              left: 15.126,
              top: -0.001,
              width: 2,
              height: 30.244,
              color: "rgb(255,255,255)",
            }}>
              <path d={"M -1 30.244 C -1 30.796 -0.552 31.244 0 31.244 C 0.552 31.244 1 30.796 1 30.244 L 0 30.244 L -1 30.244 Z M 1 0 C 1 -0.552 0.552 -1 0 -1 C -0.552 -1 -1 -0.552 -1 0 L 0 0 L 1 0 Z M 0 30.244 L 1 30.244 L 1 0 L 0 0 L -1 0 L -1 30.244 L 0 30.244 Z"} fill="currentColor" fillRule="nonzero" />
            </svg>
            <svg width={14.402} height={2} viewBox="0 -1 14.402 2" fill="none" style={{
              position: "absolute",
              left: 15.126,
              top: 15.121,
              width: 14.402,
              height: 2,
              color: "rgb(255,255,255)",
            }}>
              <path d={"M 0 -1 C -0.552 -1 -1 -0.552 -1 0 C -1 0.552 -0.552 1 0 1 L 0 0 L 0 -1 Z M 14.402 1 C 14.954 1 15.402 0.552 15.402 0 C 15.402 -0.552 14.954 -1 14.402 -1 L 14.402 0 L 14.402 1 Z M 0 0 L 0 1 L 14.402 1 L 14.402 0 L 14.402 -1 L 0 -1 L 0 0 Z"} fill="currentColor" fillRule="nonzero" />
            </svg>
          </div>
        </div>
        <div style={{
          position: "absolute",
          left: 376,
          top: 11,
          width: 24,
          height: 24,
          overflow: "hidden",
        }}>
          <div style={{
            position: "absolute",
            left: 0,
            top: 0,
            width: 24,
            height: 24,
            borderRadius: 5.6301045417785645,
            backgroundColor: "rgb(12,12,12)",
          }} />
          <svg width={9.745} height={9.750} viewBox="0 0 9.745 9.750" fill="none" style={{
            position: "absolute",
            left: 7,
            top: 7,
            width: 9.745,
            height: 9.75,
            color: "rgb(255,255,255)",
          }}>
            <path d={"M 9.422 0.408 C 9.532 0.529 9.617 0.671 9.672 0.824 C 9.727 0.978 9.752 1.141 9.744 1.305 C 9.736 1.468 9.695 1.628 9.626 1.776 C 9.556 1.923 9.457 2.056 9.336 2.166 C 8.212 3.185 7.451 4.026 6.824 5.004 C 6.191 5.993 5.66 7.173 5.053 8.916 C 4.983 9.116 4.863 9.296 4.705 9.437 C 4.546 9.577 4.354 9.675 4.147 9.721 C 3.939 9.767 3.724 9.759 3.521 9.698 C 3.317 9.637 3.133 9.524 2.985 9.372 L 0.344 6.649 C 0.117 6.412 -0.006 6.095 0 5.766 C 0.007 5.438 0.142 5.126 0.378 4.898 C 0.614 4.669 0.93 4.543 1.258 4.547 C 1.586 4.551 1.9 4.684 2.13 4.918 L 3.408 6.236 C 3.813 5.266 4.237 4.432 4.73 3.663 C 5.516 2.436 6.446 1.429 7.665 0.323 C 7.786 0.214 7.927 0.128 8.081 0.073 C 8.235 0.018 8.398 -0.006 8.561 0.001 C 8.724 0.009 8.884 0.049 9.032 0.119 C 9.18 0.189 9.313 0.287 9.422 0.408 Z"} fill="currentColor" fillRule="evenodd" />
          </svg>
        </div>
      </div>
      <div style={{
        position: "absolute",
        left: 1245,
        top: 454,
        width: 399,
        height: 333,
        overflow: "hidden",
      }}>
        <div style={{
          position: "absolute",
          left: 0,
          top: 0,
          width: 399,
          height: 333,
          borderRadius: 13.364087104797363,
          backgroundColor: "rgb(255,255,255)",
          boxShadow: "inset 0 0 0 1.322px rgba(12,12,12,0.1)",
        }} />
        <span style={{
          position: "absolute",
          left: 22,
          top: 17,
          width: 129,
          height: 23,
          fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
          fontWeight: 500,
          fontSize: 18,
          whiteSpace: "nowrap",
          lineHeight: "100%",
          color: "rgb(12,12,12)",
        }}>Recent activity</span>
        <div style={{
          position: "absolute",
          left: 22,
          top: 58,
          width: 250,
          height: 53,
          overflow: "hidden",
        }}>
          <span style={{
            position: "absolute",
            left: 58,
            top: 0,
            width: 192,
            height: 53,
            fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
            fontWeight: 400,
            fontSize: 16,
            lineHeight: 1.809999942779541,
            color: "rgb(131,131,131)",
            whiteSpace: "pre-wrap",
            display: "inline-block",
          }}><span style={{ fontWeight: 700, color: "rgb(12,12,12)" }}>{"atlas-studio-full.zip"}</span>{"\n"}<span style={{ fontSize: 14 }}>{"Export · 2.41 GB · Today, 11:42"}</span></span>
          <div style={{
            position: "absolute",
            left: 0,
            top: 0,
            transform: "matrix(-1,0,0,1,47,7)",
            transformOrigin: "0 0",
            width: 47,
            height: 44,
            borderRadius: 9.424633979797363,
            backgroundColor: "rgb(240,240,240)",
          }} />
          <div style={{
            position: "absolute",
            left: 0,
            top: 0,
            transform: "matrix(1,0,0,-1,14,39.588)",
            transformOrigin: "0 0",
            width: 20,
            height: 23.637,
            overflow: "hidden",
          }}>
            <svg width={20} height={23.637} viewBox="0 0 20 23.637" fill="none" style={{
              position: "absolute",
              left: 0,
              top: 0,
              transform: "matrix(1,0,0,-1,0,23.639)",
              transformOrigin: "0 0",
              width: 20,
              height: 23.637,
              color: "rgb(12,12,12)",
            }}>
              <path d={"M 20 10.91 L 20 21.819 C 20 22.823 19.186 23.637 18.182 23.637 L 1.818 23.637 C 0.814 23.637 0 22.823 0 21.819 L 0 10.91 C 0 9.906 0.814 9.092 1.818 9.092 L 4.545 9.092 C 5.245 9.092 5.683 9.849 5.333 10.455 C 5.17 10.737 4.87 10.91 4.545 10.91 L 1.818 10.91 L 1.818 21.819 L 18.182 21.819 L 18.182 10.91 L 15.455 10.91 C 14.755 10.91 14.317 10.152 14.667 9.546 C 14.83 9.265 15.13 9.092 15.455 9.092 L 18.182 9.092 C 19.186 9.092 20 9.906 20 10.91 Z M 6.098 6.098 L 9.091 3.104 L 9.091 13.637 C 9.091 14.337 9.848 14.774 10.455 14.424 C 10.736 14.262 10.909 13.962 10.909 13.637 L 10.909 3.104 L 13.902 6.098 C 14.397 6.594 15.243 6.367 15.424 5.691 C 15.508 5.377 15.418 5.042 15.189 4.812 L 10.643 0.267 C 10.288 -0.089 9.712 -0.089 9.357 0.267 L 4.811 4.812 C 4.316 5.307 4.543 6.153 5.219 6.334 C 5.533 6.418 5.868 6.328 6.098 6.098 Z"} fill="currentColor" fillRule="nonzero" />
            </svg>
          </div>
        </div>
        <div style={{
          position: "absolute",
          left: 22,
          top: 126,
          width: 250,
          height: 53,
          overflow: "hidden",
        }}>
          <span style={{
            position: "absolute",
            left: 58,
            top: 0,
            width: 192,
            height: 53,
            fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
            fontWeight: 400,
            fontSize: 16,
            lineHeight: 1.809999942779541,
            color: "rgb(131,131,131)",
            whiteSpace: "pre-wrap",
            display: "inline-block",
          }}><span style={{ fontWeight: 700, color: "rgb(12,12,12)" }}>{"atlas-studio-full.zip"}</span>{"\n"}<span style={{ fontSize: 14 }}>{"Export · 2.41 GB · Today, 11:42"}</span></span>
          <div style={{
            position: "absolute",
            left: 0,
            top: 0,
            transform: "matrix(-1,0,0,1,47,7)",
            transformOrigin: "0 0",
            width: 47,
            height: 44,
            borderRadius: 9.424633979797363,
            backgroundColor: "rgb(240,240,240)",
          }} />
          <div style={{
            position: "absolute",
            left: 0,
            top: 0,
            transform: "matrix(1,0,0,-1,14,39.588)",
            transformOrigin: "0 0",
            width: 20,
            height: 23.637,
            overflow: "hidden",
          }}>
            <svg width={20} height={23.637} viewBox="0 0 20 23.637" fill="none" style={{
              position: "absolute",
              left: 0,
              top: 0,
              transform: "matrix(1,0,0,-1,0,23.639)",
              transformOrigin: "0 0",
              width: 20,
              height: 23.637,
              color: "rgb(12,12,12)",
            }}>
              <path d={"M 20 10.91 L 20 21.819 C 20 22.823 19.186 23.637 18.182 23.637 L 1.818 23.637 C 0.814 23.637 0 22.823 0 21.819 L 0 10.91 C 0 9.906 0.814 9.092 1.818 9.092 L 4.545 9.092 C 5.245 9.092 5.683 9.849 5.333 10.455 C 5.17 10.737 4.87 10.91 4.545 10.91 L 1.818 10.91 L 1.818 21.819 L 18.182 21.819 L 18.182 10.91 L 15.455 10.91 C 14.755 10.91 14.317 10.152 14.667 9.546 C 14.83 9.265 15.13 9.092 15.455 9.092 L 18.182 9.092 C 19.186 9.092 20 9.906 20 10.91 Z M 6.098 6.098 L 9.091 3.104 L 9.091 13.637 C 9.091 14.337 9.848 14.774 10.455 14.424 C 10.736 14.262 10.909 13.962 10.909 13.637 L 10.909 3.104 L 13.902 6.098 C 14.397 6.594 15.243 6.367 15.424 5.691 C 15.508 5.377 15.418 5.042 15.189 4.812 L 10.643 0.267 C 10.288 -0.089 9.712 -0.089 9.357 0.267 L 4.811 4.812 C 4.316 5.307 4.543 6.153 5.219 6.334 C 5.533 6.418 5.868 6.328 6.098 6.098 Z"} fill="currentColor" fillRule="nonzero" />
            </svg>
          </div>
        </div>
        <div style={{
          position: "absolute",
          left: 22,
          top: 194,
          width: 250,
          height: 53,
          overflow: "hidden",
        }}>
          <span style={{
            position: "absolute",
            left: 58,
            top: 0,
            width: 192,
            height: 53,
            fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
            fontWeight: 400,
            fontSize: 16,
            lineHeight: 1.809999942779541,
            color: "rgb(131,131,131)",
            whiteSpace: "pre-wrap",
            display: "inline-block",
          }}><span style={{ fontWeight: 700, color: "rgb(12,12,12)" }}>{"atlas-studio-full.zip"}</span>{"\n"}<span style={{ fontSize: 14 }}>{"Export · 2.41 GB · Today, 11:42"}</span></span>
          <div style={{
            position: "absolute",
            left: 0,
            top: 0,
            transform: "matrix(-1,0,0,1,47,7)",
            transformOrigin: "0 0",
            width: 47,
            height: 44,
            borderRadius: 9.424633979797363,
            backgroundColor: "rgb(240,240,240)",
          }} />
          <div style={{
            position: "absolute",
            left: 0,
            top: 0,
            transform: "matrix(1,0,0,-1,14,39.588)",
            transformOrigin: "0 0",
            width: 20,
            height: 23.637,
            overflow: "hidden",
          }}>
            <svg width={20} height={23.637} viewBox="0 0 20 23.637" fill="none" style={{
              position: "absolute",
              left: 0,
              top: 0,
              transform: "matrix(1,0,0,-1,0,23.639)",
              transformOrigin: "0 0",
              width: 20,
              height: 23.637,
              color: "rgb(12,12,12)",
            }}>
              <path d={"M 20 10.91 L 20 21.819 C 20 22.823 19.186 23.637 18.182 23.637 L 1.818 23.637 C 0.814 23.637 0 22.823 0 21.819 L 0 10.91 C 0 9.906 0.814 9.092 1.818 9.092 L 4.545 9.092 C 5.245 9.092 5.683 9.849 5.333 10.455 C 5.17 10.737 4.87 10.91 4.545 10.91 L 1.818 10.91 L 1.818 21.819 L 18.182 21.819 L 18.182 10.91 L 15.455 10.91 C 14.755 10.91 14.317 10.152 14.667 9.546 C 14.83 9.265 15.13 9.092 15.455 9.092 L 18.182 9.092 C 19.186 9.092 20 9.906 20 10.91 Z M 6.098 6.098 L 9.091 3.104 L 9.091 13.637 C 9.091 14.337 9.848 14.774 10.455 14.424 C 10.736 14.262 10.909 13.962 10.909 13.637 L 10.909 3.104 L 13.902 6.098 C 14.397 6.594 15.243 6.367 15.424 5.691 C 15.508 5.377 15.418 5.042 15.189 4.812 L 10.643 0.267 C 10.288 -0.089 9.712 -0.089 9.357 0.267 L 4.811 4.812 C 4.316 5.307 4.543 6.153 5.219 6.334 C 5.533 6.418 5.868 6.328 6.098 6.098 Z"} fill="currentColor" fillRule="nonzero" />
            </svg>
          </div>
        </div>
        <div style={{
          position: "absolute",
          left: 21,
          top: 265,
          width: 357,
          height: 48,
          overflow: "hidden",
        }}>
          <div style={{
            position: "absolute",
            left: 0,
            top: 0,
            width: 357,
            height: 48,
            overflow: "hidden",
          }}>
            <div style={{
              position: "absolute",
              left: 0,
              top: 0,
              transform: "matrix(-1,0,0,1,357,0)",
              transformOrigin: "0 0",
              width: 357,
              height: 48,
              borderRadius: 63.3629035949707,
              boxShadow: "inset 0 0 0 0.699px rgb(131,131,131)",
            }} />
            <span style={{
              position: "absolute",
              left: 120,
              top: 13,
              width: 116,
              height: 20,
              fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
              fontWeight: 500,
              fontSize: 16,
              whiteSpace: "nowrap",
              lineHeight: "100%",
              color: "rgb(131,131,131)",
            }}>View all history</span>
          </div>
        </div>
      </div>
      <div style={{
        position: "absolute",
        left: 818,
        top: 611,
        width: 410,
        height: 136,
        overflow: "hidden",
      }}>
        <div style={{
          position: "absolute",
          left: 0,
          top: 0,
          width: 410,
          height: 136,
          borderRadius: 13.364087104797363,
          backgroundColor: "rgb(255,255,255)",
        }} />
        <span style={{
          position: "absolute",
          left: 123,
          top: 25,
          width: 195,
          height: 26,
          fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
          fontWeight: 700,
          fontSize: 20,
          whiteSpace: "nowrap",
          lineHeight: "100%",
          color: "rgb(12,12,12)",
        }}>Database &amp; Content</span>
        <span style={{
          position: "absolute",
          left: 123,
          top: 56,
          width: 192,
          height: 58,
          fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
          fontWeight: 400,
          fontSize: 16,
          lineHeight: 1.809999942779541,
          color: "rgb(131,131,131)",
          whiteSpace: "pre-wrap",
          display: "inline-block",
        }}>{"Collections and entries"}{"\n"}<span style={{ fontWeight: 700 }}>{"8 collections · 12k entries"}</span></span>
        <div style={{
          position: "absolute",
          left: 24,
          top: 30.5,
          width: 75,
          height: 75,
          overflow: "hidden",
        }}>
          <div style={{
            position: "absolute",
            left: 0,
            top: 0,
            width: 75,
            height: 75,
            borderRadius: 17.59407615661621,
            backgroundColor: "rgb(255,255,255)",
            boxShadow: "inset 0 0 0 1.173px rgba(12,12,12,0.4)",
          }} />
          <div style={{
            position: "absolute",
            left: 22.756,
            top: 22.757,
            width: 30.244,
            height: 30.244,
            overflow: "hidden",
          }}>
            <svg width={30.244} height={11.632} viewBox="0 0 30.244 11.632" fill="none" style={{
              position: "absolute",
              left: 0,
              top: -0.001,
              width: 30.244,
              height: 11.632,
              color: "rgb(12,12,12)",
            }}>
              <path d={"M 15.122 11.632 L 15.122 12.796 C 19.403 12.796 23.332 12.13 26.233 11.015 C 27.679 10.458 28.929 9.767 29.838 8.941 C 30.748 8.114 31.407 7.06 31.407 5.816 L 30.244 5.816 L 29.081 5.816 C 29.081 6.179 28.894 6.655 28.273 7.219 C 27.652 7.785 26.688 8.347 25.397 8.843 C 22.825 9.833 19.193 10.469 15.122 10.469 L 15.122 11.632 Z M 30.244 5.816 L 31.407 5.816 C 31.407 4.573 30.748 3.518 29.838 2.692 C 28.929 1.865 27.679 1.174 26.233 0.618 C 23.332 -0.498 19.403 -1.163 15.122 -1.163 L 15.122 0 L 15.122 1.163 C 19.193 1.163 22.825 1.8 25.397 2.789 C 26.688 3.285 27.652 3.848 28.273 4.413 C 28.894 4.978 29.081 5.454 29.081 5.816 L 30.244 5.816 Z M 15.122 0 L 15.122 -1.163 C 10.841 -1.163 6.912 -0.498 4.012 0.618 C 2.565 1.174 1.315 1.865 0.406 2.692 C -0.504 3.518 -1.163 4.573 -1.163 5.816 L 0 5.816 L 1.163 5.816 C 1.163 5.454 1.35 4.978 1.971 4.413 C 2.592 3.848 3.557 3.285 4.847 2.789 C 7.419 1.8 11.052 1.163 15.122 1.163 L 15.122 0 Z M 0 5.816 L -1.163 5.816 C -1.163 7.06 -0.504 8.114 0.406 8.941 C 1.315 9.767 2.565 10.458 4.012 11.015 C 6.912 12.13 10.841 12.796 15.122 12.796 L 15.122 11.632 L 15.122 10.469 C 11.052 10.469 7.419 9.833 4.847 8.843 C 3.557 8.347 2.592 7.785 1.971 7.219 C 1.35 6.655 1.163 6.179 1.163 5.816 L 0 5.816 Z"} fill="currentColor" fillRule="nonzero" />
            </svg>
            <svg width={30.244} height={24.428} viewBox="0 0 30.244 24.428" fill="none" style={{
              position: "absolute",
              left: 0,
              top: 5.815,
              width: 30.244,
              height: 24.428,
              color: "rgb(12,12,12)",
            }}>
              <path d={"M 1.163 0 C 1.163 -0.642 0.642 -1.163 0 -1.163 C -0.642 -1.163 -1.163 -0.642 -1.163 0 L 0 0 L 1.163 0 Z M 31.407 0 C 31.407 -0.642 30.887 -1.163 30.244 -1.163 C 29.602 -1.163 29.081 -0.642 29.081 0 L 30.244 0 L 31.407 0 Z M 0 0 L -1.163 0 L -1.163 18.612 L 0 18.612 L 1.163 18.612 L 1.163 0 L 0 0 Z M 0 18.612 L -1.163 18.612 C -1.163 19.855 -0.503 20.909 0.406 21.736 C 1.315 22.562 2.565 23.253 4.011 23.809 C 6.912 24.925 10.841 25.591 15.122 25.591 L 15.122 24.428 L 15.122 23.265 C 11.052 23.265 7.419 22.628 4.847 21.638 C 3.557 21.142 2.592 20.579 1.971 20.014 C 1.35 19.449 1.163 18.974 1.163 18.612 L 0 18.612 Z M 15.122 24.428 L 15.122 25.591 C 19.404 25.591 23.332 24.925 26.233 23.809 C 27.679 23.253 28.93 22.562 29.838 21.736 C 30.748 20.909 31.407 19.855 31.407 18.612 L 30.244 18.612 L 29.081 18.612 C 29.081 18.974 28.895 19.449 28.273 20.014 C 27.652 20.579 26.688 21.142 25.397 21.638 C 22.825 22.628 19.193 23.265 15.122 23.265 L 15.122 24.428 Z M 30.244 18.612 L 31.407 18.612 L 31.407 0 L 30.244 0 L 29.081 0 L 29.081 18.612 L 30.244 18.612 Z"} fill="currentColor" fillRule="nonzero" />
            </svg>
            <svg width={30.244} height={5.816} viewBox="0 0 30.244 5.816" fill="none" style={{
              position: "absolute",
              left: 0,
              top: 15.121,
              width: 30.244,
              height: 5.816,
              color: "rgb(12,12,12)",
            }}>
              <path d={"M 31.407 0 C 31.407 -0.642 30.887 -1.163 30.244 -1.163 C 29.602 -1.163 29.081 -0.642 29.081 0 L 30.244 0 L 31.407 0 Z M 1.163 0 C 1.163 -0.642 0.642 -1.163 0 -1.163 C -0.642 -1.163 -1.163 -0.642 -1.163 0 L 0 0 L 1.163 0 Z M 30.244 0 L 29.081 0 C 29.081 0.362 28.895 0.838 28.273 1.403 C 27.652 1.968 26.688 2.53 25.397 3.026 C 22.825 4.016 19.193 4.653 15.122 4.653 L 15.122 5.816 L 15.122 6.979 C 19.404 6.979 23.332 6.314 26.233 5.198 C 27.679 4.641 28.93 3.95 29.838 3.124 C 30.748 2.297 31.407 1.243 31.407 0 L 30.244 0 Z M 15.122 5.816 L 15.122 4.653 C 11.052 4.653 7.419 4.016 4.847 3.026 C 3.557 2.53 2.592 1.968 1.971 1.403 C 1.35 0.838 1.163 0.362 1.163 0 L 0 0 L -1.163 0 C -1.163 1.243 -0.503 2.297 0.406 3.124 C 1.315 3.95 2.565 4.641 4.011 5.198 C 6.912 6.314 10.841 6.979 15.122 6.979 L 15.122 5.816 Z"} fill="currentColor" fillRule="nonzero" />
            </svg>
          </div>
        </div>
        <div style={{
          position: "absolute",
          left: 376,
          top: 11,
          width: 24,
          height: 24,
          borderRadius: 5.6301045417785645,
          backgroundColor: "rgb(255,255,255)",
          boxShadow: "inset 0 0 0 1px rgba(12,12,12,0.4)",
        }} />
      </div>
      <div style={{
        position: "absolute",
        left: 392,
        top: 797,
        width: 836,
        height: 109,
        overflow: "hidden",
      }}>
        <div style={{
          position: "absolute",
          left: 0,
          top: 0,
          width: 836,
          height: 109,
          borderRadius: 13.364087104797363,
          backgroundColor: "rgb(242,242,242)",
          boxShadow: "inset 0 0 0 1.322px rgba(12,12,12,0.1)",
        }} />
        <div style={{
          position: "absolute",
          left: 617,
          top: 30,
          width: 199,
          height: 50,
          overflow: "hidden",
        }}>
          <div style={{
            position: "absolute",
            left: 0,
            top: 0,
            transform: "matrix(-1,0,0,1,199,0)",
            transformOrigin: "0 0",
            width: 199,
            height: 50,
            borderRadius: 10.828302383422852,
            backgroundColor: "rgb(12,12,12)",
          }} />
          <span style={{
            position: "absolute",
            left: 45.063,
            top: 14,
            width: 137,
            height: 22,
            fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
            fontWeight: 500,
            fontSize: 17.643409729003906,
            whiteSpace: "nowrap",
            lineHeight: "100%",
            color: "rgb(255,255,255)",
          }}>Generate export</span>
          <svg width={16} height={18.910} viewBox="0 0 16 18.910" fill="none" style={{
            position: "absolute",
            left: 21.676,
            top: 15.031,
            width: 16,
            height: 18.91,
            color: "rgb(255,255,255)",
          }}>
            <path d={"M 16 8.728 L 16 17.455 C 16 18.258 15.349 18.91 14.545 18.91 L 1.455 18.91 C 0.651 18.91 0 18.258 0 17.455 L 0 8.728 C 0 7.924 0.651 7.273 1.455 7.273 L 3.636 7.273 C 4.196 7.273 4.546 7.879 4.266 8.364 C 4.136 8.589 3.896 8.728 3.636 8.728 L 1.455 8.728 L 1.455 17.455 L 14.545 17.455 L 14.545 8.728 L 12.364 8.728 C 11.804 8.728 11.454 8.122 11.734 7.637 C 11.864 7.412 12.104 7.273 12.364 7.273 L 14.545 7.273 C 15.349 7.273 16 7.924 16 8.728 Z M 4.878 4.879 L 7.273 2.483 L 7.273 10.91 C 7.273 11.47 7.879 11.819 8.364 11.54 C 8.589 11.41 8.727 11.169 8.727 10.91 L 8.727 2.483 L 11.122 4.879 C 11.518 5.275 12.194 5.094 12.339 4.553 C 12.407 4.301 12.335 4.033 12.151 3.85 L 8.515 0.213 C 8.23 -0.071 7.77 -0.071 7.485 0.213 L 3.849 3.85 C 3.453 4.246 3.634 4.922 4.175 5.067 C 4.426 5.134 4.694 5.063 4.878 4.879 Z"} fill="currentColor" fillRule="nonzero" />
          </svg>
        </div>
        <span style={{
          position: "absolute",
          left: 100,
          top: 25,
          width: 210,
          height: 26,
          fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
          fontWeight: 700,
          fontSize: 20,
          whiteSpace: "nowrap",
          lineHeight: "100%",
          color: "rgb(12,12,12)",
        }}>Estimated bundle size</span>
        <span style={{
          position: "absolute",
          left: 100,
          top: 56,
          width: 297,
          height: 29,
          fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
          fontWeight: 400,
          fontSize: 16,
          whiteSpace: "nowrap",
          lineHeight: 1.809999942779541,
          color: "rgb(131,131,131)",
        }}>~2.6 GB · prepared in about 40 seconds</span>
        <div style={{
          position: "absolute",
          left: 41,
          top: 34.953,
          width: 34.047,
          height: 34.047,
          overflow: "hidden",
        }}>
          <svg width={34.047} height={23.571} viewBox="0 0 34.047 23.571" fill="none" style={{
            position: "absolute",
            left: 0,
            top: 10.476,
            width: 34.047,
            height: 23.571,
            color: "rgb(12,12,12)",
          }}>
            <path d={"M 5.238 24.88 C 5.961 24.88 6.547 24.294 6.547 23.571 C 6.547 22.848 5.961 22.261 5.238 22.261 L 5.238 23.571 L 5.238 24.88 Z M 0.767 22.804 L -0.159 23.73 L -0.159 23.73 L 0.767 22.804 Z M 0 0 L 0 -1.309 C -0.723 -1.309 -1.309 -0.723 -1.309 0 L 0 0 Z M 34.047 0 L 35.356 0 C 35.356 -0.723 34.77 -1.309 34.047 -1.309 L 34.047 0 Z M 28.809 22.261 C 28.086 22.261 27.499 22.848 27.499 23.571 C 27.499 24.294 28.086 24.88 28.809 24.88 L 28.809 23.571 L 28.809 22.261 Z M 5.238 23.571 L 5.238 22.261 L 2.619 22.261 L 2.619 23.571 L 2.619 24.88 L 5.238 24.88 L 5.238 23.571 Z M 2.619 23.571 L 2.619 22.261 C 2.272 22.261 1.939 22.123 1.693 21.878 L 0.767 22.804 L -0.159 23.73 C 0.578 24.466 1.577 24.88 2.619 24.88 L 2.619 23.571 Z M 0.767 22.804 L 1.693 21.878 C 1.448 21.632 1.309 21.299 1.309 20.952 L 0 20.952 L -1.309 20.952 C -1.309 21.994 -0.896 22.993 -0.159 23.73 L 0.767 22.804 Z M 0 20.952 L 1.309 20.952 L 1.309 0 L 0 0 L -1.309 0 L -1.309 20.952 L 0 20.952 Z M 0 0 L 0 1.309 L 34.047 1.309 L 34.047 0 L 34.047 -1.309 L 0 -1.309 L 0 0 Z M 34.047 0 L 32.737 0 L 32.737 20.952 L 34.047 20.952 L 35.356 20.952 L 35.356 0 L 34.047 0 Z M 34.047 20.952 L 32.737 20.952 C 32.737 21.299 32.599 21.632 32.354 21.878 L 33.28 22.804 L 34.206 23.73 C 34.942 22.993 35.356 21.994 35.356 20.952 L 34.047 20.952 Z M 33.28 22.804 L 32.354 21.878 C 32.108 22.123 31.775 22.261 31.428 22.261 L 31.428 23.571 L 31.428 24.88 C 32.47 24.88 33.469 24.466 34.206 23.73 L 33.28 22.804 Z M 31.428 23.571 L 31.428 22.261 L 28.809 22.261 L 28.809 23.571 L 28.809 24.88 L 31.428 24.88 L 31.428 23.571 Z"} fill="currentColor" fillRule="nonzero" />
          </svg>
          <svg width={13.095} height={6.547} viewBox="0 0 13.095 6.547" fill="none" style={{
            position: "absolute",
            left: 10.476,
            top: 27.499,
            width: 13.095,
            height: 6.547,
            color: "rgb(12,12,12)",
          }}>
            <path d={"M 0.926 -0.926 C 0.415 -1.437 -0.415 -1.437 -0.926 -0.926 C -1.437 -0.415 -1.437 0.415 -0.926 0.926 L 0 0 L 0.926 -0.926 Z M 6.547 6.547 L 5.622 7.473 C 6.133 7.985 6.962 7.985 7.473 7.473 L 6.547 6.547 Z M 14.021 0.926 C 14.532 0.415 14.532 -0.415 14.021 -0.926 C 13.51 -1.437 12.68 -1.437 12.169 -0.926 L 13.095 0 L 14.021 0.926 Z M 0 0 L -0.926 0.926 L 5.622 7.473 L 6.547 6.547 L 7.473 5.622 L 0.926 -0.926 L 0 0 Z M 6.547 6.547 L 7.473 7.473 L 14.021 0.926 L 13.095 0 L 12.169 -0.926 L 5.622 5.622 L 6.547 6.547 Z"} fill="currentColor" fillRule="nonzero" />
          </svg>
          <svg width={2.619} height={15.714} viewBox="-1.309 0 2.619 15.714" fill="none" style={{
            position: "absolute",
            left: 17.023,
            top: 18.333,
            width: 2.618990421295166,
            height: 15.714,
            color: "rgb(12,12,12)",
          }}>
            <path d={"M -1.309 15.714 C -1.309 16.437 -0.723 17.023 0 17.023 C 0.723 17.023 1.309 16.437 1.309 15.714 L 0 15.714 L -1.309 15.714 Z M 1.309 0 C 1.309 -0.723 0.723 -1.309 0 -1.309 C -0.723 -1.309 -1.309 -0.723 -1.309 0 L 0 0 L 1.309 0 Z M 0 15.714 L 1.309 15.714 L 1.309 0 L 0 0 L -1.309 0 L -1.309 15.714 L 0 15.714 Z"} fill="currentColor" fillRule="nonzero" />
          </svg>
          <svg width={34.047} height={10.476} viewBox="0 0 34.047 10.476" fill="none" style={{
            position: "absolute",
            left: 0,
            top: 0,
            width: 34.047,
            height: 10.476,
            color: "rgb(12,12,12)",
          }}>
            <path d={"M 28.259 1.309 L 27.125 1.964 C 27.133 1.979 27.142 1.994 27.152 2.009 L 28.259 1.309 Z M 27.33 0.368 L 28 -0.757 L 28 -0.758 L 27.33 0.368 Z M 26.059 0 L 26.093 -1.309 C 26.082 -1.309 26.07 -1.309 26.059 -1.309 L 26.059 0 Z M 7.988 0 L 7.988 -1.309 C 7.976 -1.309 7.965 -1.309 7.954 -1.309 L 7.988 0 Z M 5.788 1.309 L 6.895 2.009 C 6.905 1.994 6.914 1.979 6.922 1.964 L 5.788 1.309 Z M 0 10.476 L -1.107 9.777 C -1.362 10.18 -1.377 10.69 -1.147 11.108 C -0.916 11.526 -0.477 11.785 0 11.785 L 0 10.476 Z M 34.047 10.476 L 34.047 11.785 C 34.524 11.785 34.963 11.526 35.194 11.108 C 35.424 10.69 35.409 10.18 35.154 9.777 L 34.047 10.476 Z M 28.259 1.309 L 29.393 0.655 C 29.057 0.073 28.577 -0.414 28 -0.757 L 27.33 0.368 L 26.66 1.493 C 26.853 1.607 27.013 1.77 27.125 1.964 L 28.259 1.309 Z M 27.33 0.368 L 28 -0.758 C 27.422 -1.102 26.765 -1.291 26.093 -1.309 L 26.059 0 L 26.025 1.309 C 26.249 1.315 26.468 1.378 26.66 1.493 L 27.33 0.368 Z M 26.059 0 L 26.059 -1.309 L 7.988 -1.309 L 7.988 0 L 7.988 1.309 L 26.059 1.309 L 26.059 0 Z M 7.988 0 L 7.954 -1.309 C 7.281 -1.291 6.625 -1.102 6.047 -0.758 L 6.717 0.368 L 7.387 1.493 C 7.579 1.378 7.798 1.315 8.022 1.309 L 7.988 0 Z M 6.717 0.368 L 6.047 -0.758 C 5.469 -0.414 4.99 0.073 4.654 0.655 L 5.788 1.309 L 6.922 1.964 C 7.034 1.77 7.194 1.607 7.387 1.493 L 6.717 0.368 Z M 5.788 1.309 L 4.681 0.61 L -1.107 9.777 L 0 10.476 L 1.107 11.175 L 6.895 2.009 L 5.788 1.309 Z M 0 10.476 L 0 11.785 L 34.047 11.785 L 34.047 10.476 L 34.047 9.166 L 0 9.166 L 0 10.476 Z M 34.047 10.476 L 35.154 9.777 L 29.366 0.61 L 28.259 1.309 L 27.152 2.009 L 32.94 11.175 L 34.047 10.476 Z"} fill="currentColor" fillRule="nonzero" />
          </svg>
          <svg width={2.619} height={10.476} viewBox="-1.309 0 2.619 10.476" fill="none" style={{
            position: "absolute",
            left: 17.023,
            top: 0,
            width: 2.618990421295166,
            height: 10.476,
            color: "rgb(12,12,12)",
          }}>
            <path d={"M 1.309 0 C 1.309 -0.723 0.723 -1.309 0 -1.309 C -0.723 -1.309 -1.309 -0.723 -1.309 0 L 0 0 L 1.309 0 Z M -1.309 10.476 C -1.309 11.199 -0.723 11.785 0 11.785 C 0.723 11.785 1.309 11.199 1.309 10.476 L 0 10.476 L -1.309 10.476 Z M 0 0 L -1.309 0 L -1.309 10.476 L 0 10.476 L 1.309 10.476 L 1.309 0 L 0 0 Z"} fill="currentColor" fillRule="nonzero" />
          </svg>
        </div>
      </div>
      <div style={{
        position: "absolute",
        left: 393,
        top: 375,
        width: 540,
        height: 54,
        overflow: "hidden",
      }}>
        <span style={{
          position: "absolute",
          left: 0,
          top: 0,
          width: 249,
          height: 24,
          fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
          fontWeight: 700,
          fontSize: 20,
          textAlign: "center",
          whiteSpace: "nowrap",
          lineHeight: 1.2100000381469727,
          color: "rgb(12,12,12)",
        }}>Export workspace bundle</span>
        <span style={{
          position: "absolute",
          left: 0,
          top: 32,
          width: 540,
          height: 22,
          fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
          fontWeight: 500,
          fontSize: 18,
          textAlign: "center",
          whiteSpace: "nowrap",
          lineHeight: 1.2100000381469727,
          color: "rgb(131,131,131)",
        }}>Choose what to include. We’ll package everything as a portable</span>
      </div>
      <div style={{
        position: "absolute",
        left: 1338,
        top: 173,
        width: 124,
        height: 50,
        overflow: "hidden",
      }}>
        <div style={{
          position: "absolute",
          left: 0,
          top: 0,
          transform: "matrix(-1,0,0,1,124,0)",
          transformOrigin: "0 0",
          width: 124,
          height: 50,
          borderRadius: 10.828302383422852,
          backgroundColor: "rgba(131,131,131,0.2)",
        }} />
        <span style={{
          position: "absolute",
          left: 44,
          top: 13,
          width: 61,
          height: 22,
          fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
          fontWeight: 500,
          fontSize: 17.643409729003906,
          whiteSpace: "nowrap",
          lineHeight: "100%",
          color: "rgb(12,12,12)",
        }}>History</span>
        <svg width={18} height={18} viewBox="0 0 18 18" fill="none" style={{
          position: "absolute",
          left: 20,
          top: 15.762,
          width: 18,
          height: 18,
          color: "rgb(12,12,12)",
        }}>
          <path d={"M 8.925 18 C 6.442 18 4.333 17.121 2.6 15.362 C 0.867 13.604 0 11.475 0 8.975 L 1.5 8.975 C 1.5 11.058 2.217 12.833 3.65 14.3 C 5.083 15.767 6.842 16.5 8.925 16.5 C 11.042 16.5 12.833 15.758 14.3 14.275 C 15.767 12.792 16.5 10.992 16.5 8.875 C 16.5 6.808 15.758 5.063 14.275 3.637 C 12.792 2.212 11.008 1.5 8.925 1.5 C 7.792 1.5 6.729 1.758 5.738 2.275 C 4.746 2.792 3.883 3.475 3.15 4.325 L 5.775 4.325 L 5.775 5.825 L 0.55 5.825 L 0.55 0.625 L 2.05 0.625 L 2.05 3.275 C 2.917 2.258 3.946 1.458 5.137 0.875 C 6.329 0.292 7.592 0 8.925 0 C 10.175 0 11.35 0.233 12.45 0.7 C 13.55 1.167 14.512 1.804 15.337 2.613 C 16.163 3.421 16.813 4.367 17.288 5.45 C 17.763 6.533 18 7.7 18 8.95 C 18 10.2 17.763 11.375 17.288 12.475 C 16.813 13.575 16.163 14.533 15.337 15.35 C 14.512 16.167 13.55 16.813 12.45 17.288 C 11.35 17.763 10.175 18 8.925 18 Z M 12.125 13.075 L 8.275 9.275 L 8.275 3.925 L 9.775 3.925 L 9.775 8.65 L 13.2 12 L 12.125 13.075 Z"} fill="currentColor" fillRule="nonzero" />
        </svg>
      </div>
      <div style={{
        position: "absolute",
        left: 1471,
        top: 173,
        width: 193,
        height: 50,
        overflow: "hidden",
      }}>
        <div style={{
          position: "absolute",
          left: 0,
          top: 0,
          transform: "matrix(-1,0,0,1,193,0)",
          transformOrigin: "0 0",
          width: 193,
          height: 50,
          borderRadius: 10.828302383422852,
          boxShadow: "inset 0 0 0 1.103px rgb(131,131,131)",
        }} />
        <span style={{
          position: "absolute",
          left: 45.063,
          top: 13,
          width: 128,
          height: 22,
          fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
          fontWeight: 500,
          fontSize: 17.643409729003906,
          whiteSpace: "nowrap",
          lineHeight: "100%",
          color: "rgb(131,131,131)",
        }}>Documentation</span>
        <svg width={12.885} height={17.179} viewBox="0 0 12.885 17.179" fill="none" style={{
          position: "absolute",
          left: 0,
          top: 0,
          transform: "matrix(0.682,0.731,-0.731,0.682,31.562,13.918)",
          transformOrigin: "0 0",
          width: 12.885,
          height: 17.179,
          color: "rgb(131,131,131)",
        }}>
          <path d={"M 1.834 7.202 L 5.369 3.666 L 5.369 16.104 C 5.369 16.931 6.264 17.447 6.979 17.034 C 7.312 16.842 7.516 16.488 7.516 16.104 L 7.516 3.666 L 11.051 7.202 C 11.636 7.786 12.634 7.519 12.848 6.72 C 12.947 6.35 12.841 5.954 12.57 5.683 L 7.202 0.315 C 6.783 -0.105 6.102 -0.105 5.683 0.315 L 0.315 5.683 C -0.269 6.267 -0.002 7.266 0.797 7.48 C 1.167 7.579 1.563 7.473 1.834 7.202 Z"} fill="currentColor" fillRule="nonzero" />
        </svg>
      </div>
      <div style={{
        position: "absolute",
        left: 1506,
        top: 374,
        width: 138,
        height: 32.093,
        overflow: "hidden",
      }}>
        <div style={{
          position: "absolute",
          left: 0,
          top: 0,
          width: 138,
          height: 32.093,
          overflow: "hidden",
        }}>
          <div style={{
            position: "absolute",
            left: 0,
            top: 0,
            transform: "matrix(-1,0,0,1,138,0)",
            transformOrigin: "0 0",
            width: 138,
            height: 32.093,
            borderRadius: 63.3629035949707,
            boxShadow: "inset 0 0 0 0.699px rgb(131,131,131)",
          }} />
          <span style={{
            position: "absolute",
            left: 18.453,
            top: 7.221,
            width: 100,
            height: 18,
            fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
            fontWeight: 500,
            fontSize: 14.155759811401367,
            whiteSpace: "nowrap",
            lineHeight: "100%",
            color: "rgb(131,131,131)",
          }}>3 of 4 selected</span>
        </div>
      </div>
      <div style={{
        position: "absolute",
        left: 367,
        top: 280,
        width: 422,
        height: 50,
        overflow: "hidden",
      }}>
        <div style={{
          position: "absolute",
          left: 0,
          top: 0,
          transform: "matrix(-1,0,0,1,422,0)",
          transformOrigin: "0 0",
          width: 422,
          height: 50,
          borderRadius: 10.828302383422852,
          boxShadow: "inset 0 0 0 1.103px rgb(131,131,131)",
        }} />
        <div style={{
          position: "absolute",
          left: 0,
          top: 0,
          transform: "matrix(-1,0,0,1,112,4)",
          transformOrigin: "0 0",
          width: 107,
          height: 42,
          borderRadius: 9.424633979797363,
          backgroundColor: "rgb(108,232,255)",
        }} />
        <div style={{
          position: "absolute",
          left: 19,
          top: 14,
          width: 80,
          height: 22,
          overflow: "hidden",
        }}>
          <span style={{
            position: "absolute",
            left: 24,
            top: 0,
            width: 56,
            height: 22,
            fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
            fontWeight: 500,
            fontSize: 17.643409729003906,
            whiteSpace: "nowrap",
            lineHeight: "100%",
            color: "rgb(12,12,12)",
          }}>Export</span>
          <svg width={16} height={18.910} viewBox="0 0 16 18.910" fill="none" style={{
            position: "absolute",
            left: 0,
            top: 2.09,
            width: 16,
            height: 18.91,
            color: "rgb(12,12,12)",
          }}>
            <path d={"M 16 8.728 L 16 17.455 C 16 18.258 15.349 18.91 14.545 18.91 L 1.455 18.91 C 0.651 18.91 0 18.258 0 17.455 L 0 8.728 C 0 7.924 0.651 7.273 1.455 7.273 L 3.636 7.273 C 4.196 7.273 4.546 7.879 4.266 8.364 C 4.136 8.589 3.896 8.728 3.636 8.728 L 1.455 8.728 L 1.455 17.455 L 14.545 17.455 L 14.545 8.728 L 12.364 8.728 C 11.804 8.728 11.454 8.122 11.734 7.637 C 11.864 7.412 12.104 7.273 12.364 7.273 L 14.545 7.273 C 15.349 7.273 16 7.924 16 8.728 Z M 4.878 4.879 L 7.273 2.483 L 7.273 10.91 C 7.273 11.47 7.879 11.819 8.364 11.54 C 8.589 11.41 8.727 11.169 8.727 10.91 L 8.727 2.483 L 11.122 4.879 C 11.518 5.275 12.194 5.094 12.339 4.553 C 12.407 4.301 12.335 4.033 12.151 3.85 L 8.515 0.213 C 8.23 -0.071 7.77 -0.071 7.485 0.213 L 3.849 3.85 C 3.453 4.246 3.634 4.922 4.175 5.067 C 4.426 5.134 4.694 5.063 4.878 4.879 Z"} fill="currentColor" fillRule="nonzero" />
          </svg>
        </div>
        <div style={{
          position: "absolute",
          left: 126,
          top: 14,
          width: 79,
          height: 22,
          overflow: "hidden",
        }}>
          <span style={{
            position: "absolute",
            left: 24,
            top: 0,
            width: 55,
            height: 22,
            fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
            fontWeight: 500,
            fontSize: 17.643409729003906,
            whiteSpace: "nowrap",
            lineHeight: "100%",
            color: "rgb(12,12,12)",
          }}>Import</span>
          <svg width={16} height={18.910} viewBox="0 0 16 18.910" fill="none" style={{
            position: "absolute",
            left: 0,
            top: 0,
            transform: "matrix(1,0,0,-1,0,21.000)",
            transformOrigin: "0 0",
            width: 16,
            height: 18.91,
            color: "rgb(12,12,12)",
          }}>
            <path d={"M 16 8.728 L 16 17.455 C 16 18.258 15.349 18.91 14.545 18.91 L 1.455 18.91 C 0.651 18.91 0 18.258 0 17.455 L 0 8.728 C 0 7.924 0.651 7.273 1.455 7.273 L 3.636 7.273 C 4.196 7.273 4.546 7.879 4.266 8.364 C 4.136 8.589 3.896 8.728 3.636 8.728 L 1.455 8.728 L 1.455 17.455 L 14.545 17.455 L 14.545 8.728 L 12.364 8.728 C 11.804 8.728 11.454 8.122 11.734 7.637 C 11.864 7.412 12.104 7.273 12.364 7.273 L 14.545 7.273 C 15.349 7.273 16 7.924 16 8.728 Z M 4.878 4.879 L 7.273 2.483 L 7.273 10.91 C 7.273 11.47 7.879 11.819 8.364 11.54 C 8.589 11.41 8.727 11.169 8.727 10.91 L 8.727 2.483 L 11.122 4.879 C 11.518 5.275 12.194 5.094 12.339 4.553 C 12.407 4.301 12.335 4.033 12.151 3.85 L 8.515 0.213 C 8.23 -0.071 7.77 -0.071 7.485 0.213 L 3.849 3.85 C 3.453 4.246 3.634 4.922 4.175 5.067 C 4.426 5.134 4.694 5.063 4.878 4.879 Z"} fill="currentColor" fillRule="nonzero" />
          </svg>
        </div>
        <div style={{
          position: "absolute",
          left: 234.375,
          top: 14,
          width: 170.625,
          height: 22,
          overflow: "hidden",
        }}>
          <span style={{
            position: "absolute",
            left: 24.625,
            top: 0,
            width: 146,
            height: 22,
            fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
            fontWeight: 500,
            fontSize: 17.643409729003906,
            whiteSpace: "nowrap",
            lineHeight: "100%",
            color: "rgb(12,12,12)",
          }}>Clone Workspace</span>
          <div style={{
            position: "absolute",
            left: 0,
            top: 1.141,
            width: 17.25,
            height: 17.247,
            overflow: "hidden",
          }}>
            <svg width={13.417} height={13.417} viewBox="0 0 13.417 13.417" fill="none" style={{
              position: "absolute",
              left: 3.833,
              top: 0,
              width: 13.417,
              height: 13.417,
              color: "rgb(0,0,0)",
            }}>
              <path d={"M 0 2.556 L -1 2.556 L 0 2.556 Z M 2.556 0 L 2.556 -1 L 2.556 0 Z M 10.861 0 L 10.861 -1 L 10.861 0 Z M 13.417 10.861 L 12.417 10.861 L 13.417 10.861 Z M 0 2.556 L 1 2.556 C 1 2.143 1.164 1.747 1.456 1.456 L 0.749 0.749 L 0.041 0.041 C -0.625 0.708 -1 1.613 -1 2.556 L 0 2.556 Z M 0.749 0.749 L 1.456 1.456 C 1.747 1.164 2.143 1 2.556 1 L 2.556 0 L 2.556 -1 C 1.613 -1 0.708 -0.625 0.041 0.041 L 0.749 0.749 Z M 2.556 0 L 2.556 1 L 10.861 1 L 10.861 0 L 10.861 -1 L 2.556 -1 L 2.556 0 Z M 10.861 0 L 10.861 1 C 11.273 1 11.669 1.164 11.961 1.456 L 12.668 0.749 L 13.375 0.041 C 12.708 -0.625 11.804 -1 10.861 -1 L 10.861 0 Z M 12.668 0.749 L 11.961 1.456 C 12.253 1.747 12.417 2.143 12.417 2.556 L 13.417 2.556 L 14.417 2.556 C 14.417 1.613 14.042 0.708 13.375 0.041 L 12.668 0.749 Z M 13.417 2.556 L 12.417 2.556 L 12.417 10.861 L 13.417 10.861 L 14.417 10.861 L 14.417 2.556 L 13.417 2.556 Z M 13.417 10.861 L 12.417 10.861 C 12.417 11.273 12.253 11.669 11.961 11.961 L 12.668 12.668 L 13.375 13.375 C 14.042 12.708 14.417 11.804 14.417 10.861 L 13.417 10.861 Z M 12.668 12.668 L 11.961 11.961 C 11.669 12.253 11.273 12.417 10.861 12.417 L 10.861 13.417 L 10.861 14.417 C 11.804 14.417 12.708 14.042 13.375 13.375 L 12.668 12.668 Z M 10.861 13.417 L 10.861 12.417 L 2.556 12.417 L 2.556 13.417 L 2.556 14.417 L 10.861 14.417 L 10.861 13.417 Z M 2.556 13.417 L 2.556 12.417 C 2.143 12.417 1.747 12.253 1.456 11.961 L 0.749 12.668 L 0.041 13.375 C 0.708 14.042 1.613 14.417 2.556 14.417 L 2.556 13.417 Z M 0.749 12.668 L 1.456 11.961 C 1.164 11.669 1 11.273 1 10.861 L 0 10.861 L -1 10.861 C -1 11.804 -0.625 12.708 0.041 13.375 L 0.749 12.668 Z M 0 10.861 L 1 10.861 L 1 2.556 L 0 2.556 L -1 2.556 L -1 10.861 L 0 10.861 Z"} fill="currentColor" fillRule="nonzero" />
            </svg>
            <svg width={12.938} height={13.165} viewBox="0 0 12.938 13.165" fill="none" style={{
              position: "absolute",
              left: 0,
              top: 4.082,
              width: 12.938,
              height: 13.165,
              color: "rgb(0,0,0)",
            }}>
              <path d={"M 1.465 0.869 C 1.945 0.595 2.112 -0.015 1.839 -0.495 C 1.565 -0.975 0.954 -1.142 0.475 -0.869 L 0.97 0 L 1.465 0.869 Z M 0 1.665 L -1 1.662 L -1 1.665 L 0 1.665 Z M 13.811 12.692 C 14.08 12.21 13.906 11.601 13.424 11.332 C 12.941 11.064 12.332 11.238 12.064 11.72 L 12.938 12.206 L 13.811 12.692 Z M 0.97 0 L 0.475 -0.869 C 0.028 -0.614 -0.344 -0.246 -0.603 0.199 L 0.261 0.702 L 1.125 1.205 C 1.207 1.065 1.324 0.949 1.465 0.869 L 0.97 0 Z M 0.261 0.702 L -0.603 0.199 C -0.862 0.643 -0.999 1.148 -1 1.662 L 0 1.665 L 1 1.667 C 1 1.505 1.044 1.346 1.125 1.205 L 0.261 0.702 Z M 0 1.665 L -1 1.665 L -1 11.248 L 0 11.248 L 1 11.248 L 1 1.665 L 0 1.665 Z M 0 11.248 L -1 11.248 C -1 12.854 0.31 14.165 1.917 14.165 L 1.917 13.165 L 1.917 12.165 C 1.415 12.165 1 11.75 1 11.248 L 0 11.248 Z M 1.917 13.165 L 1.917 14.165 L 11.5 14.165 L 11.5 13.165 L 11.5 12.165 L 1.917 12.165 L 1.917 13.165 Z M 11.5 13.165 L 11.5 14.165 C 12.047 14.165 12.535 14.019 12.953 13.716 C 13.349 13.428 13.614 13.047 13.811 12.692 L 12.938 12.206 L 12.064 11.72 C 11.933 11.955 11.839 12.053 11.778 12.097 C 11.738 12.126 11.672 12.165 11.5 12.165 L 11.5 13.165 Z"} fill="currentColor" fillRule="nonzero" />
            </svg>
            <svg width={5.750} height={2} viewBox="0 -1 5.750 2" fill="none" style={{
              position: "absolute",
              left: 7.667,
              top: 6.708,
              width: 5.75,
              height: 2,
              color: "rgb(0,0,0)",
            }}>
              <path d={"M 0 -1 C -0.552 -1 -1 -0.552 -1 0 C -1 0.552 -0.552 1 0 1 L 0 0 L 0 -1 Z M 5.75 1 C 6.302 1 6.75 0.552 6.75 0 C 6.75 -0.552 6.302 -1 5.75 -1 L 5.75 0 L 5.75 1 Z M 0 0 L 0 1 L 5.75 1 L 5.75 0 L 5.75 -1 L 0 -1 L 0 0 Z"} fill="currentColor" fillRule="nonzero" />
            </svg>
          </div>
        </div>
      </div>
      <svg width={1350} height={1} viewBox="0 -0.500 1350 1" fill="none" style={{
        position: "absolute",
        left: 337,
        top: 261,
        width: 1350,
        height: 1,
        color: "rgba(131,131,131,0.25)",
      }}>
        <path d={"M 0 -0.5 C -0.276 -0.5 -0.5 -0.276 -0.5 0 C -0.5 0.276 -0.276 0.5 0 0.5 L 0 0 L 0 -0.5 Z M 1350 0.5 C 1350.276 0.5 1350.5 0.276 1350.5 0 C 1350.5 -0.276 1350.276 -0.5 1350 -0.5 L 1350 0 L 1350 0.5 Z M 0 0 L 0 0.5 L 1350 0.5 L 1350 0 L 1350 -0.5 L 0 -0.5 L 0 0 Z"} fill="currentColor" fillRule="nonzero" />
      </svg>
      <span style={{
        position: "absolute",
        left: 367,
        top: 198,
        width: 590,
        height: 44,
        fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
        fontWeight: 400,
        fontSize: 18,
        lineHeight: 1.2000000476837158,
        color: "rgb(131,131,131)",
      }}>Move assets, templates and settings between workspaces. Bundle a complete copy or clone an entire environment in one step.</span>
      <span style={{
        position: "absolute",
        left: 367,
        top: 164,
        width: 263,
        height: 26,
        fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
        fontWeight: 600,
        fontSize: 20,
        whiteSpace: "nowrap",
        lineHeight: "100%",
        color: "rgb(12,12,12)",
      }}>Import &amp; Export Workspace</span>
      <div style={{
        position: "absolute",
        left: 41,
        top: 144,
        width: 285,
        height: 420,
        overflow: "hidden",
      }}>
        <div style={{
          position: "absolute",
          left: 0,
          top: 0,
          width: 285,
          height: 420,
          borderRadius: 15.55555534362793,
          backgroundColor: "rgb(255,255,255)",
        }} />
        <div style={{
          position: "absolute",
          left: 0,
          top: 0,
          transform: "matrix(-1,0,0,1,275,240)",
          transformOrigin: "0 0",
          width: 266,
          height: 50,
          borderRadius: 10,
          background: "linear-gradient(90deg, rgba(255,255,255,0) 0.00%, rgba(148,238,255,0.4) 67.15%, rgba(108,232,255,0.4) 100.00%)",
        }} />
        <span style={{
          position: "absolute",
          left: 52.114,
          top: 23,
          width: 81,
          height: 23,
          fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
          fontWeight: 500,
          fontSize: 18,
          whiteSpace: "nowrap",
          lineHeight: "100%",
          color: "rgb(131,131,131)",
        }}>Overview</span>
        <div style={{
          position: "absolute",
          left: 23.114,
          top: 24,
          width: 19,
          height: 19.003,
          overflow: "hidden",
        }}>
          <svg width={19} height={19} viewBox="0 0 19 19" fill="none" style={{
            position: "absolute",
            left: 0,
            top: 0,
            width: 19,
            height: 19,
            color: "rgb(131,131,131)",
          }}>
            <path d={"M 0.319 14.468 L 1.152 14.379 L 1.152 14.379 L 0.319 14.468 Z M 4.503 18.663 L 4.41 19.496 L 4.41 19.496 L 4.503 18.663 Z M 14.497 18.663 L 14.59 19.496 L 14.59 19.496 L 14.497 18.663 Z M 18.681 14.468 L 17.847 14.379 L 17.847 14.379 L 18.681 14.468 Z M 18.681 4.532 L 17.847 4.621 L 17.847 4.621 L 18.681 4.532 Z M 14.497 0.337 L 14.404 1.17 L 14.404 1.17 L 14.497 0.337 Z M 4.503 0.337 L 4.596 1.17 L 4.596 1.17 L 4.503 0.337 Z M 0.319 4.532 L 1.152 4.621 L 1.152 4.621 L 0.319 4.532 Z M 0.319 14.468 L -0.514 14.557 C -0.238 17.14 1.833 19.209 4.41 19.496 L 4.503 18.663 L 4.596 17.83 C 2.796 17.629 1.345 16.176 1.152 14.379 L 0.319 14.468 Z M 4.503 18.663 L 4.41 19.496 C 6.043 19.678 7.75 19.838 9.5 19.838 L 9.5 19 L 9.5 18.162 C 7.846 18.162 6.215 18.01 4.596 17.83 L 4.503 18.663 Z M 9.5 19 L 9.5 19.838 C 11.25 19.838 12.957 19.678 14.59 19.496 L 14.497 18.663 L 14.404 17.83 C 12.785 18.01 11.154 18.162 9.5 18.162 L 9.5 19 Z M 14.497 18.663 L 14.59 19.496 C 17.167 19.209 19.238 17.14 19.514 14.557 L 18.681 14.468 L 17.847 14.379 C 17.655 16.176 16.204 17.629 14.404 17.83 L 14.497 18.663 Z M 18.681 14.468 L 19.514 14.557 C 19.688 12.933 19.838 11.238 19.838 9.5 L 19 9.5 L 18.162 9.5 C 18.162 11.146 18.02 12.77 17.847 14.379 L 18.681 14.468 Z M 19 9.5 L 19.838 9.5 C 19.838 7.762 19.688 6.067 19.514 4.443 L 18.681 4.532 L 17.847 4.621 C 18.02 6.23 18.162 7.854 18.162 9.5 L 19 9.5 Z M 18.681 4.532 L 19.514 4.443 C 19.238 1.86 17.167 -0.209 14.59 -0.496 L 14.497 0.337 L 14.404 1.17 C 16.204 1.371 17.655 2.824 17.847 4.621 L 18.681 4.532 Z M 14.497 0.337 L 14.59 -0.496 C 12.957 -0.678 11.25 -0.838 9.5 -0.838 L 9.5 0 L 9.5 0.838 C 11.154 0.838 12.785 0.99 14.404 1.17 L 14.497 0.337 Z M 9.5 0 L 9.5 -0.838 C 7.75 -0.838 6.043 -0.678 4.41 -0.496 L 4.503 0.337 L 4.596 1.17 C 6.215 0.99 7.846 0.838 9.5 0.838 L 9.5 0 Z M 4.503 0.337 L 4.41 -0.496 C 1.833 -0.209 -0.238 1.86 -0.514 4.443 L 0.319 4.532 L 1.152 4.621 C 1.345 2.824 2.796 1.371 4.596 1.17 L 4.503 0.337 Z M 0.319 4.532 L -0.514 4.443 C -0.688 6.067 -0.838 7.762 -0.838 9.5 L 0 9.5 L 0.838 9.5 C 0.838 7.854 0.98 6.23 1.152 4.621 L 0.319 4.532 Z M 0 9.5 L -0.838 9.5 C -0.838 11.238 -0.688 12.933 -0.514 14.557 L 0.319 14.468 L 1.152 14.379 C 0.98 12.77 0.838 11.146 0.838 9.5 L 0 9.5 Z"} fill="currentColor" fillRule="nonzero" />
          </svg>
          <svg width={18.448} height={4.940} viewBox="0 0 18.448 4.940" fill="none" style={{
            position: "absolute",
            left: 0.276,
            top: 0,
            width: 18.448,
            height: 4.94,
            color: "rgb(131,131,131)",
          }}>
            <path d={"M 18.448 4.94 L 18.448 5.778 C 18.685 5.778 18.911 5.678 19.07 5.501 C 19.229 5.325 19.306 5.09 19.282 4.854 L 18.448 4.94 Z M 0 4.94 L -0.834 4.854 C -0.858 5.09 -0.781 5.325 -0.622 5.501 C -0.463 5.678 -0.237 5.778 0 5.778 L 0 4.94 Z M 0.043 4.532 L 0.876 4.621 L 0.876 4.621 L 0.043 4.532 Z M 4.227 0.337 L 4.32 1.17 L 4.32 1.17 L 4.227 0.337 Z M 14.221 0.337 L 14.314 -0.496 L 14.314 -0.496 L 14.221 0.337 Z M 18.405 4.532 L 19.239 4.443 L 19.238 4.443 L 18.405 4.532 Z M 18.448 4.94 L 18.448 4.102 L 0 4.102 L 0 4.94 L 0 5.778 L 18.448 5.778 L 18.448 4.94 Z M 0 4.94 L 0.834 5.026 C 0.848 4.891 0.862 4.756 0.876 4.621 L 0.043 4.532 L -0.791 4.443 C -0.805 4.579 -0.82 4.716 -0.834 4.854 L 0 4.94 Z M 0.043 4.532 L 0.876 4.621 C 1.069 2.824 2.52 1.371 4.32 1.17 L 4.227 0.337 L 4.134 -0.496 C 1.557 -0.209 -0.514 1.86 -0.791 4.443 L 0.043 4.532 Z M 4.227 0.337 L 4.32 1.17 C 5.939 0.99 7.57 0.838 9.224 0.838 L 9.224 0 L 9.224 -0.838 C 7.474 -0.838 5.767 -0.678 4.134 -0.496 L 4.227 0.337 Z M 9.224 0 L 9.224 0.838 C 10.878 0.838 12.509 0.99 14.128 1.17 L 14.221 0.337 L 14.314 -0.496 C 12.681 -0.678 10.974 -0.838 9.224 -0.838 L 9.224 0 Z M 14.221 0.337 L 14.128 1.17 C 15.928 1.371 17.379 2.824 17.572 4.621 L 18.405 4.532 L 19.238 4.443 C 18.962 1.86 16.891 -0.209 14.314 -0.496 L 14.221 0.337 Z M 18.405 4.532 L 17.571 4.62 C 17.586 4.756 17.6 4.891 17.614 5.026 L 18.448 4.94 L 19.282 4.854 C 19.267 4.716 19.253 4.58 19.239 4.443 L 18.405 4.532 Z"} fill="currentColor" fillRule="nonzero" />
          </svg>
          <svg width={12.473} height={5.347} viewBox="0 0 12.473 5.347" fill="none" style={{
            position: "absolute",
            left: 3.262,
            top: 13.656,
            width: 12.473,
            height: 5.347,
            color: "rgb(131,131,131)",
          }}>
            <path d={"M 12.473 4.696 L 12.788 5.473 C 13.185 5.312 13.397 4.877 13.278 4.465 L 12.473 4.696 Z M 0 4.696 L -0.806 4.465 C -0.924 4.877 -0.712 5.312 -0.315 5.473 L 0 4.696 Z M 1.24 5.01 L 1.332 4.177 L 1.332 4.177 L 1.24 5.01 Z M 11.233 5.01 L 11.14 4.177 L 11.14 4.177 L 11.233 5.01 Z M 12.473 4.696 L 13.278 4.465 C 12.401 1.403 9.581 -0.838 6.236 -0.838 L 6.236 0 L 6.236 0.838 C 8.814 0.838 10.99 2.565 11.667 4.927 L 12.473 4.696 Z M 6.236 0 L 6.236 -0.838 C 2.891 -0.838 0.072 1.403 -0.806 4.465 L 0 4.696 L 0.806 4.927 C 1.483 2.565 3.659 0.838 6.236 0.838 L 6.236 0 Z M 0 4.696 L -0.315 5.473 C 0.146 5.659 0.636 5.786 1.147 5.843 L 1.24 5.01 L 1.332 4.177 C 0.978 4.137 0.636 4.049 0.315 3.919 L 0 4.696 Z M 1.24 5.01 L 1.147 5.843 C 2.78 6.025 4.486 6.185 6.236 6.185 L 6.236 5.347 L 6.236 4.508 C 4.583 4.508 2.951 4.357 1.332 4.177 L 1.24 5.01 Z M 6.236 5.347 L 6.236 6.185 C 7.986 6.185 9.693 6.025 11.326 5.843 L 11.233 5.01 L 11.14 4.177 C 9.522 4.357 7.89 4.508 6.236 4.508 L 6.236 5.347 Z M 11.233 5.01 L 11.326 5.843 C 11.837 5.786 12.327 5.659 12.788 5.473 L 12.473 4.696 L 12.158 3.919 C 11.837 4.049 11.495 4.137 11.14 4.177 L 11.233 5.01 Z"} fill="currentColor" fillRule="nonzero" />
          </svg>
          <svg width={2.691} height={4.737} viewBox="0 0 2.691 4.737" fill="none" style={{
            position: "absolute",
            left: 10.758,
            top: 9.04,
            width: 2.691,
            height: 4.737,
            color: "rgb(131,131,131)",
          }}>
            <path d={"M -0.729 4.323 C -0.958 4.725 -0.817 5.237 -0.414 5.465 C -0.012 5.694 0.5 5.553 0.729 5.151 L 0 4.737 L -0.729 4.323 Z M 3.42 0.414 C 3.649 0.012 3.508 -0.5 3.105 -0.729 C 2.703 -0.958 2.191 -0.817 1.962 -0.414 L 2.691 0 L 3.42 0.414 Z M 0 4.737 L 0.729 5.151 L 3.42 0.414 L 2.691 0 L 1.962 -0.414 L -0.729 4.323 L 0 4.737 Z"} fill="currentColor" fillRule="nonzero" />
          </svg>
        </div>
        <span style={{
          position: "absolute",
          left: 52.114,
          top: 81,
          width: 86,
          height: 23,
          fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
          fontWeight: 500,
          fontSize: 18,
          whiteSpace: "nowrap",
          lineHeight: "100%",
          color: "rgb(131,131,131)",
        }}>Brand kits</span>
        <div style={{
          position: "absolute",
          left: 23.114,
          top: 81.809,
          width: 19,
          height: 19.384,
          overflow: "hidden",
        }}>
          <svg width={2.982} height={2.982} viewBox="0 0 2.982 2.982" fill="none" style={{
            position: "absolute",
            left: 10.239,
            top: 3.724,
            width: 2.982,
            height: 2.982,
            color: "rgb(131,131,131)",
          }}>
            <path d={"M 1.491 2.982 L 1.491 3.728 C 2.726 3.728 3.728 2.726 3.728 1.491 L 2.982 1.491 L 2.237 1.491 C 2.237 1.903 1.903 2.237 1.491 2.237 L 1.491 2.982 Z M 2.982 1.491 L 3.728 1.491 C 3.728 0.256 2.726 -0.746 1.491 -0.746 L 1.491 0 L 1.491 0.746 C 1.903 0.746 2.237 1.079 2.237 1.491 L 2.982 1.491 Z M 1.491 0 L 1.491 -0.746 C 0.256 -0.746 -0.746 0.256 -0.746 1.491 L 0 1.491 L 0.746 1.491 C 0.746 1.079 1.079 0.746 1.491 0.746 L 1.491 0 Z M 0 1.491 L -0.746 1.491 C -0.746 2.726 0.256 3.728 1.491 3.728 L 1.491 2.982 L 1.491 2.237 C 1.079 2.237 0.746 1.903 0.746 1.491 L 0 1.491 Z"} fill="currentColor" fillRule="nonzero" />
          </svg>
          <svg width={1.491} height={1.491} viewBox="0 0 1.491 1.491" fill="none" style={{
            position: "absolute",
            left: 5.021,
            top: 12.675,
            width: 1.491,
            height: 1.491,
            color: "rgb(131,131,131)",
          }}>
            <path d={"M 0.746 1.491 L 0.746 2.237 C 1.569 2.237 2.237 1.569 2.237 0.746 L 1.491 0.746 L 0.746 0.746 L 0.746 1.491 Z M 1.491 0.746 L 2.237 0.746 C 2.237 -0.078 1.569 -0.746 0.746 -0.746 L 0.746 0 L 0.746 0.746 L 1.491 0.746 Z M 0.746 0 L 0.746 -0.746 C -0.078 -0.746 -0.746 -0.078 -0.746 0.746 L 0 0.746 L 0.746 0.746 L 0.746 0 Z M 0 0.746 L -0.746 0.746 C -0.746 1.569 -0.078 2.237 0.746 2.237 L 0.746 1.491 L 0.746 0.746 L 0 0.746 Z"} fill="currentColor" fillRule="nonzero" />
          </svg>
          <svg width={2.982} height={2.982} viewBox="0 0 2.982 2.982" fill="none" style={{
            position: "absolute",
            left: 4.275,
            top: 5.966,
            width: 2.982,
            height: 2.982,
            color: "rgb(131,131,131)",
          }}>
            <path d={"M 1.491 2.982 L 1.491 3.728 C 2.726 3.728 3.728 2.726 3.728 1.491 L 2.982 1.491 L 2.237 1.491 C 2.237 1.903 1.903 2.237 1.491 2.237 L 1.491 2.982 Z M 2.982 1.491 L 3.728 1.491 C 3.728 0.256 2.726 -0.746 1.491 -0.746 L 1.491 0 L 1.491 0.746 C 1.903 0.746 2.237 1.079 2.237 1.491 L 2.982 1.491 Z M 1.491 0 L 1.491 -0.746 C 0.256 -0.746 -0.746 0.256 -0.746 1.491 L 0 1.491 L 0.746 1.491 C 0.746 1.079 1.079 0.746 1.491 0.746 L 1.491 0 Z M 0 1.491 L -0.746 1.491 C -0.746 2.726 0.256 3.728 1.491 3.728 L 1.491 2.982 L 1.491 2.237 C 1.079 2.237 0.746 1.903 0.746 1.491 L 0 1.491 Z"} fill="currentColor" fillRule="nonzero" />
          </svg>
          <svg width={19} height={19.384} viewBox="0 0 19 19.384" fill="none" style={{
            position: "absolute",
            left: 0,
            top: -0.002,
            width: 19,
            height: 19.384,
            color: "rgb(131,131,131)",
          }}>
            <path d={"M 13.251 17.565 L 12.507 17.607 C 12.508 17.624 12.509 17.642 12.512 17.658 L 13.251 17.565 Z M 12.962 16.764 L 13.562 16.321 L 13.561 16.321 L 12.962 16.764 Z M 12.282 16.252 L 12.542 15.554 C 12.533 15.55 12.524 15.547 12.515 15.544 L 12.282 16.252 Z M 10.665 14.969 L 10.028 15.357 L 10.028 15.357 L 10.665 14.969 Z M 10.269 12.944 L 11.005 13.063 L 11.005 13.063 L 10.269 12.944 Z M 13.221 10.437 L 13.219 11.183 L 13.221 11.183 L 13.221 10.437 Z M 16.01 10.437 L 16.012 9.692 L 16.01 9.692 L 16.01 10.437 Z M 17.384 10.106 L 17.726 10.769 L 17.726 10.768 L 17.384 10.106 Z M 18.452 9.178 L 19.06 9.609 L 19.06 9.609 L 18.452 9.178 Z M 18.972 7.863 L 19.71 7.965 L 19.71 7.965 L 18.972 7.863 Z M 18.828 6.456 L 18.125 6.705 L 18.125 6.706 L 18.828 6.456 Z M 15.816 2.18 L 15.345 2.758 L 15.345 2.758 L 15.816 2.18 Z M 11.021 0.092 L 11.123 -0.647 L 11.123 -0.647 L 11.021 0.092 Z M 5.838 0.799 L 5.542 0.115 L 5.542 0.115 L 5.838 0.799 Z M 1.778 4.097 L 2.387 4.527 L 2.387 4.527 L 1.778 4.097 Z M 0.023 9.024 L -0.721 8.972 L -0.721 8.972 L 0.023 9.024 Z M 1.084 14.145 L 0.422 14.488 L 0.422 14.488 L 1.084 14.145 Z M 4.652 17.97 L 5.039 17.333 L 5.039 17.333 L 4.652 17.97 Z M 9.687 19.384 L 9.69 18.638 L 9.688 18.638 L 9.687 19.384 Z M 12.282 19.026 L 12.081 18.308 L 12.078 18.309 L 12.282 19.026 Z M 13.251 17.565 L 13.995 17.522 C 13.97 17.088 13.82 16.671 13.562 16.321 L 12.962 16.764 L 12.362 17.207 C 12.448 17.324 12.498 17.463 12.507 17.607 L 13.251 17.565 Z M 12.962 16.764 L 13.561 16.321 C 13.303 15.972 12.949 15.705 12.542 15.554 L 12.282 16.252 L 12.022 16.951 C 12.158 17.002 12.276 17.091 12.362 17.207 L 12.962 16.764 Z M 12.282 16.252 L 12.515 15.544 C 12.009 15.378 11.579 15.037 11.302 14.582 L 10.665 14.969 L 10.028 15.357 C 10.49 16.115 11.206 16.684 12.049 16.961 L 12.282 16.252 Z M 10.665 14.969 L 11.302 14.582 C 11.026 14.127 10.92 13.588 11.005 13.063 L 10.269 12.944 L 9.533 12.825 C 9.392 13.701 9.567 14.599 10.028 15.357 L 10.665 14.969 Z M 10.269 12.944 L 11.005 13.063 C 11.09 12.537 11.36 12.059 11.765 11.715 L 11.283 11.146 L 10.8 10.578 C 10.124 11.152 9.675 11.949 9.533 12.825 L 10.269 12.944 Z M 11.283 11.146 L 11.765 11.715 C 12.171 11.37 12.687 11.181 13.219 11.183 L 13.221 10.437 L 13.223 9.692 C 12.336 9.689 11.477 10.004 10.8 10.578 L 11.283 11.146 Z M 13.221 10.437 L 13.221 11.183 L 16.01 11.183 L 16.01 10.437 L 16.01 9.692 L 13.221 9.692 L 13.221 10.437 Z M 16.01 10.437 L 16.007 11.183 C 16.605 11.185 17.195 11.042 17.726 10.769 L 17.384 10.106 L 17.043 9.443 C 16.724 9.608 16.37 9.693 16.012 9.692 L 16.01 10.437 Z M 17.384 10.106 L 17.726 10.768 C 18.257 10.495 18.715 10.097 19.06 9.609 L 18.452 9.178 L 17.843 8.748 C 17.636 9.04 17.362 9.279 17.043 9.443 L 17.384 10.106 Z M 18.452 9.178 L 19.06 9.609 C 19.406 9.121 19.629 8.557 19.71 7.965 L 18.972 7.863 L 18.233 7.761 C 18.184 8.116 18.051 8.455 17.843 8.748 L 18.452 9.178 Z M 18.972 7.863 L 19.71 7.965 C 19.792 7.373 19.73 6.77 19.53 6.206 L 18.828 6.456 L 18.125 6.706 C 18.245 7.044 18.283 7.406 18.233 7.761 L 18.972 7.863 Z M 18.828 6.456 L 19.53 6.207 C 18.892 4.404 17.77 2.811 16.287 1.602 L 15.816 2.18 L 15.345 2.758 C 16.616 3.794 17.577 5.159 18.125 6.705 L 18.828 6.456 Z M 15.816 2.18 L 16.287 1.602 C 14.804 0.393 13.018 -0.385 11.123 -0.647 L 11.021 0.092 L 10.918 0.83 C 12.543 1.055 14.074 1.722 15.345 2.758 L 15.816 2.18 Z M 11.021 0.092 L 11.123 -0.647 C 9.228 -0.909 7.297 -0.646 5.542 0.115 L 5.838 0.799 L 6.135 1.483 C 7.639 0.831 9.294 0.605 10.918 0.83 L 11.021 0.092 Z M 5.838 0.799 L 5.542 0.115 C 3.786 0.876 2.274 2.104 1.169 3.666 L 1.778 4.097 L 2.387 4.527 C 3.334 3.188 4.63 2.135 6.135 1.483 L 5.838 0.799 Z M 1.778 4.097 L 1.169 3.666 C 0.065 5.228 -0.589 7.064 -0.721 8.972 L 0.023 9.024 L 0.767 9.075 C 0.88 7.439 1.44 5.866 2.387 4.527 L 1.778 4.097 Z M 0.023 9.024 L -0.721 8.972 C -0.853 10.881 -0.457 12.789 0.422 14.488 L 1.084 14.145 L 1.746 13.803 C 0.993 12.346 0.654 10.711 0.767 9.075 L 0.023 9.024 Z M 1.084 14.145 L 0.422 14.488 C 1.301 16.187 2.63 17.612 4.264 18.607 L 4.652 17.97 L 5.039 17.333 C 3.639 16.481 2.5 15.259 1.746 13.803 L 1.084 14.145 Z M 4.652 17.97 L 4.264 18.607 C 5.898 19.602 7.774 20.128 9.687 20.129 L 9.687 19.384 L 9.688 18.638 C 8.048 18.637 6.44 18.186 5.039 17.333 L 4.652 17.97 Z M 9.687 19.384 L 9.685 20.129 C 10.632 20.132 11.575 20.002 12.486 19.743 L 12.282 19.026 L 12.078 18.309 C 11.301 18.53 10.497 18.64 9.69 18.638 L 9.687 19.384 Z M 12.282 19.026 L 12.483 19.744 C 12.965 19.609 13.383 19.305 13.659 18.887 L 13.038 18.475 L 12.417 18.063 C 12.338 18.183 12.218 18.269 12.081 18.308 L 12.282 19.026 Z M 13.038 18.475 L 13.659 18.887 C 13.936 18.47 14.054 17.967 13.991 17.471 L 13.251 17.565 L 12.512 17.658 C 12.53 17.8 12.496 17.944 12.417 18.063 L 13.038 18.475 Z"} fill="currentColor" fillRule="nonzero" />
          </svg>
        </div>
        <span style={{
          position: "absolute",
          left: 52.114,
          top: 139,
          width: 211,
          height: 23,
          fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
          fontWeight: 500,
          fontSize: 18,
          whiteSpace: "nowrap",
          lineHeight: "100%",
          color: "rgb(131,131,131)",
        }}>Engagement Intelligence</span>
        <svg width={19} height={19.915} viewBox="0 0 19 19.915" fill="none" style={{
          position: "absolute",
          left: 23,
          top: 139.039,
          width: 19,
          height: 19.915,
          color: "rgb(131,131,131)",
        }}>
          <path d={"M 8.116 11.501 L 8.165 12.248 C 8.178 12.351 8.217 12.435 8.284 12.5 C 8.352 12.564 8.434 12.596 8.532 12.596 L 9.338 12.596 C 9.436 12.596 9.519 12.564 9.587 12.5 C 9.656 12.435 9.696 12.351 9.709 12.248 L 9.759 11.501 C 9.958 11.468 10.144 11.398 10.318 11.291 C 10.492 11.183 10.645 11.063 10.779 10.929 L 11.526 11.177 C 11.609 11.211 11.692 11.211 11.775 11.177 C 11.858 11.144 11.923 11.087 11.969 11.007 L 12.352 10.352 C 12.399 10.272 12.418 10.19 12.41 10.107 C 12.401 10.024 12.358 9.948 12.281 9.879 L 11.775 9.485 C 11.858 9.252 11.899 9.012 11.899 8.763 C 11.899 8.514 11.858 8.273 11.775 8.041 L 12.281 7.647 C 12.358 7.578 12.401 7.501 12.41 7.418 C 12.418 7.335 12.399 7.254 12.352 7.173 L 11.969 6.519 C 11.923 6.438 11.858 6.381 11.775 6.348 C 11.692 6.315 11.609 6.315 11.526 6.348 L 10.779 6.597 C 10.641 6.469 10.482 6.354 10.302 6.252 C 10.122 6.15 9.941 6.074 9.759 6.024 L 9.709 5.278 C 9.696 5.174 9.657 5.09 9.59 5.026 C 9.523 4.961 9.44 4.929 9.342 4.929 L 8.536 4.929 C 8.438 4.929 8.355 4.961 8.287 5.026 C 8.218 5.09 8.178 5.174 8.165 5.278 L 8.116 6.024 C 7.933 6.074 7.752 6.15 7.572 6.252 C 7.392 6.354 7.233 6.469 7.095 6.597 L 6.348 6.348 C 6.265 6.315 6.182 6.315 6.099 6.348 C 6.016 6.381 5.951 6.438 5.905 6.519 L 5.522 7.173 C 5.475 7.254 5.456 7.335 5.464 7.418 C 5.473 7.501 5.516 7.578 5.593 7.647 L 6.099 8.041 C 6.016 8.273 5.975 8.514 5.975 8.763 C 5.975 9.012 6.016 9.252 6.099 9.485 L 5.593 9.879 C 5.516 9.948 5.473 10.024 5.464 10.107 C 5.456 10.19 5.475 10.272 5.522 10.352 L 5.905 11.007 C 5.951 11.087 6.016 11.144 6.099 11.177 C 6.182 11.211 6.265 11.211 6.348 11.177 L 7.095 10.929 C 7.229 11.063 7.382 11.183 7.556 11.291 C 7.73 11.398 7.916 11.468 8.116 11.501 Z M 8.94 10.505 C 8.457 10.505 8.045 10.336 7.705 9.998 C 7.365 9.66 7.194 9.249 7.194 8.766 C 7.194 8.282 7.364 7.871 7.702 7.53 C 8.04 7.19 8.451 7.02 8.934 7.02 C 9.417 7.02 9.829 7.189 10.169 7.528 C 10.509 7.866 10.68 8.276 10.68 8.76 C 10.68 9.243 10.51 9.655 10.172 9.995 C 9.834 10.335 9.423 10.505 8.94 10.505 Z M 2.987 15.634 C 2.041 14.771 1.307 13.762 0.784 12.609 C 0.261 11.455 0 10.24 0 8.962 C 0 6.472 0.871 4.356 2.614 2.614 C 4.356 0.871 6.472 0 8.962 0 C 11.036 0 12.874 0.61 14.476 1.83 C 16.078 3.05 17.119 4.639 17.6 6.597 L 18.969 12.024 C 19.036 12.256 18.994 12.468 18.845 12.659 C 18.696 12.85 18.496 12.945 18.247 12.945 L 15.932 12.945 L 15.932 16.43 C 15.932 16.841 15.786 17.193 15.494 17.485 C 15.201 17.778 14.849 17.924 14.439 17.924 L 11.949 17.924 L 11.949 19.169 C 11.949 19.38 11.877 19.557 11.733 19.701 C 11.589 19.844 11.411 19.915 11.198 19.915 C 10.985 19.915 10.808 19.844 10.667 19.701 C 10.526 19.557 10.456 19.38 10.456 19.169 L 10.456 17.177 C 10.456 16.965 10.527 16.788 10.67 16.645 C 10.813 16.502 10.991 16.43 11.202 16.43 L 14.439 16.43 L 14.439 12.198 C 14.439 11.987 14.51 11.809 14.653 11.666 C 14.797 11.523 14.974 11.451 15.185 11.451 L 17.277 11.451 L 16.156 6.97 C 15.758 5.361 14.887 4.045 13.542 3.025 C 12.198 2.004 10.671 1.494 8.962 1.494 C 6.887 1.494 5.124 2.211 3.672 3.647 C 2.22 5.083 1.494 6.834 1.494 8.901 C 1.494 9.97 1.712 10.985 2.149 11.947 C 2.586 12.91 3.206 13.765 4.008 14.513 L 4.481 14.961 L 4.481 19.169 C 4.481 19.38 4.409 19.557 4.265 19.701 C 4.121 19.844 3.943 19.915 3.73 19.915 C 3.517 19.915 3.34 19.844 3.199 19.701 C 3.058 19.557 2.987 19.38 2.987 19.169 L 2.987 15.634 Z"} fill="currentColor" fillRule="nonzero" />
        </svg>
        <span style={{
          position: "absolute",
          left: 52.114,
          top: 197,
          width: 175,
          height: 23,
          fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
          fontWeight: 500,
          fontSize: 18,
          whiteSpace: "nowrap",
          lineHeight: "100%",
          color: "rgb(131,131,131)",
        }}>Account Connection</span>
        <div style={{
          position: "absolute",
          left: 23.114,
          top: 197.977,
          width: 19,
          height: 18.998,
          overflow: "hidden",
        }}>
          <svg width={12.214} height={4.524} viewBox="0 0 12.214 4.524" fill="none" style={{
            position: "absolute",
            left: 0,
            top: 10.412,
            width: 12.214,
            height: 4.524,
            color: "rgb(131,131,131)",
          }}>
            <path d={"M 0 4.524 L -0.708 4.524 L -0.708 5.232 L 0 5.232 L 0 4.524 Z M 0 1.991 L -0.344 1.372 L -0.708 1.574 L -0.708 1.991 L 0 1.991 Z M 7.691 0 L 7.689 0.708 L 7.691 0.708 L 7.691 0 Z M 9.048 4.524 L 9.048 3.816 L 0 3.816 L 0 4.524 L 0 5.232 L 9.048 5.232 L 9.048 4.524 Z M 0 4.524 L 0.708 4.524 L 0.708 1.991 L 0 1.991 L -0.708 1.991 L -0.708 4.524 L 0 4.524 Z M 0 1.991 L 0.344 2.609 C 2.59 1.359 5.119 0.704 7.689 0.708 L 7.691 0 L 7.692 -0.708 C 4.879 -0.712 2.113 0.004 -0.344 1.372 L 0 1.991 Z M 7.691 0 L 7.691 0.708 C 9.193 0.708 10.644 0.927 12.012 1.335 L 12.214 0.656 L 12.417 -0.023 C 10.918 -0.469 9.331 -0.708 7.691 -0.708 L 7.691 0 Z"} fill="currentColor" fillRule="nonzero" />
          </svg>
          <svg width={7.691} height={7.691} viewBox="0 0 7.691 7.691" fill="none" style={{
            position: "absolute",
            left: 4.071,
            top: 0,
            width: 7.691,
            height: 7.691,
            color: "rgb(131,131,131)",
          }}>
            <path d={"M 0 3.845 L -0.708 3.845 L 0 3.845 Z M 3.845 0 L 3.845 -0.708 L 3.845 0 Z M 0 3.845 L -0.708 3.845 C -0.708 5.053 -0.228 6.211 0.626 7.065 L 1.126 6.564 L 1.627 6.064 C 1.039 5.475 0.708 4.677 0.708 3.845 L 0 3.845 Z M 1.126 6.564 L 0.626 7.065 C 1.479 7.919 2.638 8.399 3.845 8.399 L 3.845 7.691 L 3.845 6.982 C 3.013 6.982 2.215 6.652 1.627 6.064 L 1.126 6.564 Z M 3.845 7.691 L 3.845 8.399 C 5.053 8.399 6.211 7.919 7.065 7.065 L 6.564 6.564 L 6.064 6.064 C 5.475 6.652 4.677 6.982 3.845 6.982 L 3.845 7.691 Z M 6.564 6.564 L 7.065 7.065 C 7.919 6.211 8.399 5.053 8.399 3.845 L 7.691 3.845 L 6.982 3.845 C 6.982 4.677 6.652 5.475 6.064 6.064 L 6.564 6.564 Z M 7.691 3.845 L 8.399 3.845 C 8.399 2.638 7.919 1.479 7.065 0.626 L 6.564 1.126 L 6.064 1.627 C 6.652 2.215 6.982 3.013 6.982 3.845 L 7.691 3.845 Z M 6.564 1.126 L 7.065 0.626 C 6.211 -0.228 5.053 -0.708 3.845 -0.708 L 3.845 0 L 3.845 0.708 C 4.677 0.708 5.475 1.039 6.064 1.627 L 6.564 1.126 Z M 3.845 0 L 3.845 -0.708 C 2.638 -0.708 1.479 -0.228 0.626 0.626 L 1.126 1.126 L 1.627 1.627 C 2.215 1.039 3.013 0.708 3.845 0.708 L 3.845 0 Z M 1.126 1.126 L 0.626 0.626 C -0.228 1.479 -0.708 2.638 -0.708 3.845 L 0 3.845 L 0.708 3.845 C 0.708 3.013 1.039 2.215 1.627 1.627 L 1.126 1.126 Z"} fill="currentColor" fillRule="nonzero" />
          </svg>
          <svg width={8.143} height={8.143} viewBox="0 0 8.143 8.143" fill="none" style={{
            position: "absolute",
            left: 10.857,
            top: 10.857,
            width: 8.143,
            height: 8.143,
            color: "rgb(131,131,131)",
          }}>
            <path d={"M 8.143 4.071 L 8.143 3.363 L 0 3.363 L 0 4.071 L 0 4.78 L 8.143 4.78 L 8.143 4.071 Z M 4.071 0 L 3.363 0 L 3.363 8.143 L 4.071 8.143 L 4.78 8.143 L 4.78 0 L 4.071 0 Z"} fill="currentColor" fillRule="nonzero" />
          </svg>
        </div>
        <span style={{
          position: "absolute",
          left: 52.114,
          top: 255,
          width: 131,
          height: 23,
          fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
          fontWeight: 500,
          fontSize: 18,
          whiteSpace: "nowrap",
          lineHeight: "100%",
          color: "rgb(12,12,12)",
        }}>Import / Export</span>
        <div style={{
          position: "absolute",
          left: 23.114,
          top: 256.742,
          width: 19,
          height: 17.782,
          overflow: "hidden",
        }}>
          <svg width={19} height={4.071} viewBox="0 0 19 4.071" fill="none" style={{
            position: "absolute",
            left: 0,
            top: 13.711,
            width: 19,
            height: 4.071,
            color: "rgb(12,12,12)",
          }}>
            <path d={"M 17.643 0 L 17.643 2.714 L 1.357 2.714 L 1.357 0 L 0 0 L 0 2.714 L 0.005 2.711 C 0.005 2.889 0.039 3.066 0.107 3.23 C 0.174 3.395 0.274 3.545 0.399 3.671 C 0.525 3.798 0.674 3.898 0.838 3.967 C 1.003 4.035 1.179 4.071 1.357 4.071 L 17.643 4.071 C 18.003 4.071 18.348 3.928 18.603 3.674 C 18.857 3.419 19 3.074 19 2.714 L 19 0 L 17.643 0 Z"} fill="currentColor" fillRule="nonzero" />
          </svg>
          <svg width={8.143} height={12.214} viewBox="0 0 8.143 12.214" fill="none" style={{
            position: "absolute",
            left: 9.821,
            top: 0,
            width: 8.143,
            height: 12.214,
            color: "rgb(12,12,12)",
          }}>
            <path d={"M 7.193 7.193 L 4.75 9.636 L 4.75 0 L 3.393 0 L 3.393 9.636 L 0.95 7.193 L 0 8.143 L 4.071 12.214 L 8.143 8.143 L 7.193 7.193 Z"} fill="currentColor" fillRule="nonzero" />
          </svg>
          <svg width={8.143} height={12.214} viewBox="0 0 8.143 12.214" fill="none" style={{
            position: "absolute",
            left: 0.679,
            top: 0,
            width: 8.143,
            height: 12.214,
            color: "rgb(12,12,12)",
          }}>
            <path d={"M 4.071 0 L 0 4.071 L 0.95 5.021 L 3.393 2.579 L 3.393 12.214 L 4.75 12.214 L 4.75 2.579 L 7.193 5.021 L 8.143 4.071 L 4.071 0 Z"} fill="currentColor" fillRule="nonzero" />
          </svg>
        </div>
        <span style={{
          position: "absolute",
          left: 52.114,
          top: 313,
          width: 129,
          height: 23,
          fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
          fontWeight: 500,
          fontSize: 18,
          whiteSpace: "nowrap",
          lineHeight: "100%",
          color: "rgb(131,131,131)",
        }}>Credit &amp; Usage</span>
        <svg width={20.702} height={14.680} viewBox="0 0 20.702 14.680" fill="none" style={{
          position: "absolute",
          left: 0,
          top: 0,
          transform: "matrix(0.859,-0.512,0.512,0.859,19.987,322.748)",
          transformOrigin: "0 0",
          width: 20.702,
          height: 14.68,
          color: "rgb(131,131,131)",
        }}>
          <path d={"M 17.755 1.471 C 15.831 0.509 13.268 0 10.351 0 C 7.434 0 4.872 0.509 2.947 1.471 C 1.074 2.407 0 3.724 0 5.081 L 0 9.598 C 0 10.956 1.074 12.273 2.947 13.209 C 4.872 14.171 7.434 14.68 10.351 14.68 C 13.268 14.68 15.831 14.171 17.755 13.209 C 19.629 12.273 20.702 10.956 20.702 9.598 L 20.702 5.081 C 20.702 3.724 19.629 2.407 17.755 1.471 Z M 3.452 2.481 C 5.195 1.609 7.646 1.129 10.351 1.129 C 13.057 1.129 15.507 1.609 17.251 2.481 C 18.726 3.219 19.573 4.167 19.573 5.081 C 19.573 5.996 18.726 6.944 17.251 7.681 C 15.507 8.554 13.057 9.034 10.351 9.034 C 7.646 9.034 5.195 8.554 3.452 7.681 C 1.976 6.944 1.129 5.996 1.129 5.081 C 1.129 4.167 1.976 3.219 3.452 2.481 Z M 9.787 10.156 L 9.787 13.544 C 7.976 13.499 6.298 13.235 4.893 12.781 L 4.893 9.45 C 6.33 9.874 7.984 10.115 9.787 10.156 Z M 10.916 10.156 C 12.718 10.115 14.372 9.874 15.809 9.449 L 15.809 12.78 C 14.404 13.234 12.726 13.498 10.916 13.543 L 10.916 10.156 Z M 1.129 9.598 L 1.129 7.434 C 1.666 7.945 2.28 8.369 2.947 8.691 C 3.208 8.821 3.48 8.943 3.764 9.056 L 3.764 12.35 C 3.658 12.301 3.553 12.256 3.452 12.201 C 1.976 11.461 1.129 10.513 1.129 9.598 Z M 17.251 12.198 C 17.149 12.249 17.045 12.298 16.938 12.347 L 16.938 9.053 C 17.221 8.94 17.493 8.818 17.755 8.688 C 18.422 8.367 19.036 7.944 19.573 7.434 L 19.573 9.598 C 19.573 10.513 18.726 11.461 17.251 12.198 Z"} fill="currentColor" fillRule="nonzero" />
        </svg>
        <span style={{
          position: "absolute",
          left: 52.114,
          top: 371,
          width: 97,
          height: 23,
          fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
          fontWeight: 500,
          fontSize: 18,
          whiteSpace: "nowrap",
          lineHeight: "100%",
          color: "rgb(131,131,131)",
        }}>Team Roles</span>
        <div style={{
          position: "absolute",
          left: 23.114,
          top: 372.5,
          width: 19,
          height: 19,
          overflow: "hidden",
        }}>
          <svg width={4.750} height={4.750} viewBox="0 0 4.750 4.750" fill="none" style={{
            position: "absolute",
            left: 2.375,
            top: 0,
            width: 4.75,
            height: 4.75,
            color: "rgb(131,131,131)",
          }}>
            <path d={"M 2.375 0 L 2.375 -0.792 L 2.375 0 Z M 0 2.375 L -0.792 2.375 C -0.792 3.215 -0.458 4.02 0.136 4.614 L 0.696 4.054 L 1.255 3.495 C 0.958 3.198 0.792 2.795 0.792 2.375 L 0 2.375 Z M 0.696 4.054 L 0.136 4.614 C 0.73 5.208 1.535 5.542 2.375 5.542 L 2.375 4.75 L 2.375 3.958 C 1.955 3.958 1.552 3.792 1.255 3.495 L 0.696 4.054 Z M 2.375 4.75 L 2.375 5.542 C 3.215 5.542 4.02 5.208 4.614 4.614 L 4.054 4.054 L 3.495 3.495 C 3.198 3.792 2.795 3.958 2.375 3.958 L 2.375 4.75 Z M 4.054 4.054 L 4.614 4.614 C 5.208 4.02 5.542 3.215 5.542 2.375 L 4.75 2.375 L 3.958 2.375 C 3.958 2.795 3.792 3.198 3.495 3.495 L 4.054 4.054 Z M 4.75 2.375 L 5.542 2.375 C 5.542 1.535 5.208 0.73 4.614 0.136 L 4.054 0.696 L 3.495 1.255 C 3.792 1.552 3.958 1.955 3.958 2.375 L 4.75 2.375 Z M 4.054 0.696 L 4.614 0.136 C 4.02 -0.458 3.215 -0.792 2.375 -0.792 L 2.375 0 L 2.375 0.792 C 2.795 0.792 3.198 0.958 3.495 1.255 L 4.054 0.696 Z M 2.375 0 L 2.375 -0.792 C 1.535 -0.792 0.73 -0.458 0.136 0.136 L 0.696 0.696 L 1.255 1.255 C 1.552 0.958 1.955 0.792 2.375 0.792 L 2.375 0 Z M 0.696 0.696 L 0.136 0.136 C -0.458 0.73 -0.792 1.535 -0.792 2.375 L 0 2.375 L 0.792 2.375 C 0.792 1.955 0.958 1.552 1.255 1.255 L 0.696 0.696 Z"} fill="currentColor" fillRule="nonzero" />
          </svg>
          <svg width={4.750} height={4.750} viewBox="0 0 4.750 4.750" fill="none" style={{
            position: "absolute",
            left: 0,
            top: 7.125,
            width: 4.75,
            height: 4.75,
            color: "rgb(131,131,131)",
          }}>
            <path d={"M -0.792 4.75 C -0.792 5.187 -0.437 5.542 0 5.542 C 0.437 5.542 0.792 5.187 0.792 4.75 L 0 4.75 L -0.792 4.75 Z M 4.75 0.792 C 5.187 0.792 5.542 0.437 5.542 0 C 5.542 -0.437 5.187 -0.792 4.75 -0.792 L 4.75 0 L 4.75 0.792 Z M 0 4.75 L 0.792 4.75 C 0.792 2.564 2.564 0.792 4.75 0.792 L 4.75 0 L 4.75 -0.792 C 1.689 -0.792 -0.792 1.689 -0.792 4.75 L 0 4.75 Z"} fill="currentColor" fillRule="nonzero" />
          </svg>
          <svg width={4.750} height={4.750} viewBox="0 0 4.750 4.750" fill="none" style={{
            position: "absolute",
            left: 14.25,
            top: 7.125,
            width: 4.75,
            height: 4.75,
            color: "rgb(131,131,131)",
          }}>
            <path d={"M 3.958 4.75 C 3.958 5.187 4.313 5.542 4.75 5.542 C 5.187 5.542 5.542 5.187 5.542 4.75 L 4.75 4.75 L 3.958 4.75 Z M 0 -0.792 C -0.437 -0.792 -0.792 -0.437 -0.792 0 C -0.792 0.437 -0.437 0.792 0 0.792 L 0 0 L 0 -0.792 Z M 4.75 4.75 L 5.542 4.75 C 5.542 1.689 3.061 -0.792 0 -0.792 L 0 0 L 0 0.792 C 2.186 0.792 3.958 2.564 3.958 4.75 L 4.75 4.75 Z"} fill="currentColor" fillRule="nonzero" />
          </svg>
          <svg width={4.750} height={4.750} viewBox="0 0 4.750 4.750" fill="none" style={{
            position: "absolute",
            left: 11.875,
            top: 0,
            width: 4.75,
            height: 4.75,
            color: "rgb(131,131,131)",
          }}>
            <path d={"M 0 2.375 L -0.792 2.375 L 0 2.375 Z M 4.75 2.375 L 5.542 2.375 L 4.75 2.375 Z M 2.375 0 L 2.375 -0.792 L 2.375 0 Z M 0 2.375 L -0.792 2.375 C -0.792 3.215 -0.458 4.02 0.136 4.614 L 0.696 4.054 L 1.255 3.495 C 0.958 3.198 0.792 2.795 0.792 2.375 L 0 2.375 Z M 0.696 4.054 L 0.136 4.614 C 0.73 5.208 1.535 5.542 2.375 5.542 L 2.375 4.75 L 2.375 3.958 C 1.955 3.958 1.552 3.792 1.255 3.495 L 0.696 4.054 Z M 2.375 4.75 L 2.375 5.542 C 3.215 5.542 4.02 5.208 4.614 4.614 L 4.054 4.054 L 3.495 3.495 C 3.198 3.792 2.795 3.958 2.375 3.958 L 2.375 4.75 Z M 4.054 4.054 L 4.614 4.614 C 5.208 4.02 5.542 3.215 5.542 2.375 L 4.75 2.375 L 3.958 2.375 C 3.958 2.795 3.792 3.198 3.495 3.495 L 4.054 4.054 Z M 4.75 2.375 L 5.542 2.375 C 5.542 1.535 5.208 0.73 4.614 0.136 L 4.054 0.696 L 3.495 1.255 C 3.792 1.552 3.958 1.955 3.958 2.375 L 4.75 2.375 Z M 4.054 0.696 L 4.614 0.136 C 4.02 -0.458 3.215 -0.792 2.375 -0.792 L 2.375 0 L 2.375 0.792 C 2.795 0.792 3.198 0.958 3.495 1.255 L 4.054 0.696 Z M 2.375 0 L 2.375 -0.792 C 1.535 -0.792 0.73 -0.458 0.136 0.136 L 0.696 0.696 L 1.255 1.255 C 1.552 0.958 1.955 0.792 2.375 0.792 L 2.375 0 Z M 0.696 0.696 L 0.136 0.136 C -0.458 0.73 -0.792 1.535 -0.792 2.375 L 0 2.375 L 0.792 2.375 C 0.792 1.955 0.958 1.552 1.255 1.255 L 0.696 0.696 Z"} fill="currentColor" fillRule="nonzero" />
          </svg>
          <svg width={9.500} height={4.750} viewBox="0 0 9.500 4.750" fill="none" style={{
            position: "absolute",
            left: 4.75,
            top: 14.25,
            width: 9.5,
            height: 4.75,
            color: "rgb(131,131,131)",
          }}>
            <path d={"M 8.708 4.75 C 8.708 5.187 9.063 5.542 9.5 5.542 C 9.937 5.542 10.292 5.187 10.292 4.75 L 9.5 4.75 L 8.708 4.75 Z M -0.792 4.75 C -0.792 5.187 -0.437 5.542 0 5.542 C 0.437 5.542 0.792 5.187 0.792 4.75 L 0 4.75 L -0.792 4.75 Z M 9.5 4.75 L 10.292 4.75 C 10.292 1.689 7.811 -0.792 4.75 -0.792 L 4.75 0 L 4.75 0.792 C 6.936 0.792 8.708 2.564 8.708 4.75 L 9.5 4.75 Z M 4.75 0 L 4.75 -0.792 C 1.689 -0.792 -0.792 1.689 -0.792 4.75 L 0 4.75 L 0.792 4.75 C 0.792 2.564 2.564 0.792 4.75 0.792 L 4.75 0 Z"} fill="currentColor" fillRule="nonzero" />
          </svg>
          <svg width={4.750} height={4.750} viewBox="0 0 4.750 4.750" fill="none" style={{
            position: "absolute",
            left: 7.125,
            top: 7.125,
            width: 4.75,
            height: 4.75,
            color: "rgb(131,131,131)",
          }}>
            <path d={"M 0 2.375 L 0.792 2.375 L 0.792 2.375 L 0 2.375 Z M 4.75 2.375 L 3.958 2.375 L 3.958 2.375 L 4.75 2.375 Z M 2.375 0 L 2.375 0.792 L 2.375 0 Z M 0 2.375 L -0.792 2.375 C -0.792 2.791 -0.71 3.203 -0.551 3.587 L 0.181 3.284 L 0.912 2.981 C 0.833 2.789 0.792 2.583 0.792 2.375 L 0 2.375 Z M 0.181 3.284 L -0.551 3.587 C -0.391 3.971 -0.158 4.32 0.136 4.614 L 0.696 4.054 L 1.255 3.495 C 1.108 3.348 0.992 3.173 0.912 2.981 L 0.181 3.284 Z M 0.696 4.054 L 0.136 4.614 C 0.43 4.908 0.779 5.141 1.163 5.301 L 1.466 4.569 L 1.769 3.838 C 1.577 3.758 1.402 3.642 1.255 3.495 L 0.696 4.054 Z M 1.466 4.569 L 1.163 5.301 C 1.547 5.46 1.959 5.542 2.375 5.542 L 2.375 4.75 L 2.375 3.958 C 2.167 3.958 1.961 3.917 1.769 3.838 L 1.466 4.569 Z M 2.375 4.75 L 2.375 5.542 C 2.791 5.542 3.203 5.46 3.587 5.301 L 3.284 4.569 L 2.981 3.838 C 2.789 3.917 2.583 3.958 2.375 3.958 L 2.375 4.75 Z M 3.284 4.569 L 3.587 5.301 C 3.971 5.141 4.32 4.908 4.614 4.614 L 4.054 4.054 L 3.495 3.495 C 3.348 3.642 3.173 3.758 2.981 3.838 L 3.284 4.569 Z M 4.054 4.054 L 4.614 4.614 C 4.908 4.32 5.141 3.971 5.301 3.587 L 4.569 3.284 L 3.838 2.981 C 3.758 3.173 3.642 3.348 3.495 3.495 L 4.054 4.054 Z M 4.569 3.284 L 5.301 3.587 C 5.46 3.203 5.542 2.791 5.542 2.375 L 4.75 2.375 L 3.958 2.375 C 3.958 2.583 3.917 2.789 3.838 2.981 L 4.569 3.284 Z M 4.75 2.375 L 5.542 2.375 C 5.542 1.959 5.46 1.547 5.301 1.163 L 4.569 1.466 L 3.838 1.769 C 3.917 1.961 3.958 2.167 3.958 2.375 L 4.75 2.375 Z M 4.569 1.466 L 5.301 1.163 C 5.141 0.779 4.908 0.43 4.614 0.136 L 4.054 0.696 L 3.495 1.255 C 3.642 1.402 3.758 1.577 3.838 1.769 L 4.569 1.466 Z M 4.054 0.696 L 4.614 0.136 C 4.32 -0.158 3.971 -0.391 3.587 -0.551 L 3.284 0.181 L 2.981 0.912 C 3.173 0.992 3.348 1.108 3.495 1.255 L 4.054 0.696 Z M 3.284 0.181 L 3.587 -0.551 C 3.203 -0.71 2.791 -0.792 2.375 -0.792 L 2.375 0 L 2.375 0.792 C 2.583 0.792 2.789 0.833 2.981 0.912 L 3.284 0.181 Z M 2.375 0 L 2.375 -0.792 C 1.959 -0.792 1.547 -0.71 1.163 -0.551 L 1.466 0.181 L 1.769 0.912 C 1.961 0.833 2.167 0.792 2.375 0.792 L 2.375 0 Z M 1.466 0.181 L 1.163 -0.551 C 0.779 -0.391 0.43 -0.158 0.136 0.136 L 0.696 0.696 L 1.255 1.255 C 1.402 1.108 1.577 0.992 1.769 0.912 L 1.466 0.181 Z M 0.696 0.696 L 0.136 0.136 C -0.158 0.43 -0.391 0.779 -0.551 1.163 L 0.181 1.466 L 0.912 1.769 C 0.992 1.577 1.108 1.402 1.255 1.255 L 0.696 0.696 Z M 0.181 1.466 L -0.551 1.163 C -0.71 1.547 -0.792 1.959 -0.792 2.375 L 0 2.375 L 0.792 2.375 C 0.792 2.167 0.833 1.961 0.912 1.769 L 0.181 1.466 Z"} fill="currentColor" fillRule="nonzero" />
          </svg>
        </div>
      </div>
      <div style={{
        position: "absolute",
        left: 1491,
        top: 33.969,
        width: 196,
        height: 70.216,
        overflow: "hidden",
      }}>
        <svg width={126} height={47} viewBox="0 0 126 47" fill="none" style={{
          position: "absolute",
          left: 70,
          top: 14.031,
          width: 126,
          height: 47,
          borderRadius: 12.507452964782715,
          backdropFilter: "blur(17.220px)",
          color: "rgb(255,255,255)",
        }}>
          <path d={"M 9.159 7.596 C 9.159 3.401 12.56 0 16.755 0 L 113.493 0 C 120.4 0 126 5.6 126 12.507 L 126 34.493 C 126 41.4 120.4 47 113.493 47 L 20.305 47 C 14.181 47 9.246 41.979 9.353 35.856 L 9.361 35.382 C 9.408 32.652 8.322 30.025 6.36 28.126 C 2.742 24.623 2.345 18.956 5.439 14.982 L 7.557 12.262 C 8.596 10.929 9.159 9.286 9.159 7.596 Z"} fill="currentColor" fillRule="nonzero" />
        </svg>
        <div style={{
          position: "absolute",
          left: 0,
          top: 0,
          width: 70.216,
          height: 70.216,
          overflow: "hidden",
        }}>
          <div style={{
            position: "absolute",
            left: 0,
            top: 0,
            width: 70.216,
            height: 70.216,
            overflow: "hidden",
          }}>
            <div style={{
              position: "absolute",
              left: 0,
              top: 0,
              width: 70.216,
              height: 70.216,
              overflow: "hidden",
            }}>
              <div style={{
                position: "absolute",
                left: 0,
                top: 0,
                width: 70.21607971191406,
                height: 70.21607971191406,
                clipPath: "ellipse(35.108px 35.108px at 35.108px 35.108px)",
              }}>
                <div style={{
                  position: "absolute",
                  left: 0,
                  top: 0,
                  width: 70.216,
                  height: 70.216,
                  borderRadius: "50%",
                  backgroundColor: "rgba(108,232,255,0.2)",
                  backdropFilter: "blur(33.686px)",
                  boxShadow: "inset 0px 0px 10.731px 3.860px rgba(255,255,255,0.84)",
                }} />
                <div style={{
                  position: "absolute",
                  left: 14.322,
                  top: 19.27,
                  width: 41.803,
                  height: 37.554,
                  overflow: "hidden",
                }}>
                  <div style={{
                    position: "absolute",
                    left: 15.677,
                    top: 0,
                    width: 18.82,
                    height: 18.82,
                    borderRadius: "50%",
                    backgroundColor: "rgb(245,107,255)",
                  }} />
                  <div style={{
                    position: "absolute",
                    left: 0,
                    top: 2.852,
                    width: 27.433,
                    height: 27.433,
                    borderRadius: "50%",
                    backgroundColor: "rgb(108,232,255)",
                  }} />
                  <div style={{
                    position: "absolute",
                    left: 13.777,
                    top: 21.852,
                    width: 15.702,
                    height: 15.702,
                    borderRadius: "50%",
                    backgroundColor: "rgb(163,65,255)",
                  }} />
                  <div style={{
                    position: "absolute",
                    left: 26.101,
                    top: 11.973,
                    width: 15.702,
                    height: 15.702,
                    borderRadius: "50%",
                    backgroundColor: "rgb(255,255,255)",
                  }} />
                </div>
                <div style={{
                  position: "absolute",
                  left: 13.989,
                  top: 25.996,
                  width: 42.881,
                  height: 17.384,
                  borderRadius: 12.974635124206543,
                  backgroundColor: "rgba(255,255,255,0.07)",
                  backdropFilter: "blur(11.718px)",
                  boxShadow: "inset 0px 0px 6.530px -1.159px rgb(108,232,255)",
                }} />
              </div>
            </div>
            <div style={{
              position: "absolute",
              left: 20.363,
              top: 31.199,
              width: 7.022,
              height: 7.022,
              borderRadius: 14.043211936950684,
              backgroundColor: "rgb(255,255,255)",
            }} />
            <div style={{
              position: "absolute",
              left: 44.236,
              top: 31.199,
              width: 7.022,
              height: 7.022,
              borderRadius: 14.043211936950684,
              backgroundColor: "rgb(255,255,255)",
            }} />
          </div>
        </div>
        <span style={{
          position: "absolute",
          left: 102,
          top: 28.031,
          width: 75,
          height: 18,
          fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
          fontWeight: 600,
          fontSize: 14,
          whiteSpace: "nowrap",
          lineHeight: "100%",
          color: "rgb(12,12,12)",
        }}>Ask Spark?</span>
      </div>
      <div style={{
        position: "absolute",
        left: 1033,
        top: 44,
        width: 435,
        height: 50,
        overflow: "hidden",
      }}>
        <div style={{
          position: "absolute",
          left: 0,
          top: 0,
          transform: "matrix(-1,0,0,1,435,0)",
          transformOrigin: "0 0",
          width: 435,
          height: 50,
          borderRadius: 92.40505981445312,
          backgroundColor: "rgb(255,255,255)",
          boxShadow: "0px -100px 40px 0px rgba(0,0,0,0.1)",
        }} />
        <div style={{
          position: "absolute",
          left: 0,
          top: 0,
          transform: "matrix(-1,0,0,1,229,4)",
          transformOrigin: "0 0",
          width: 224,
          height: 42,
          borderRadius: 76.38336181640625,
          backgroundColor: "rgb(12,12,12)",
        }} />
        <div style={{
          position: "absolute",
          left: 25.255,
          top: 14,
          width: 183.496,
          height: 21,
          overflow: "hidden",
        }}>
          <span style={{
            position: "absolute",
            left: 24.496,
            top: 0,
            width: 159,
            height: 21,
            fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
            fontWeight: 500,
            fontSize: 16.63400650024414,
            whiteSpace: "nowrap",
            lineHeight: "100%",
            color: "rgb(255,255,255)",
          }}>Workspace Settings</span>
          <div style={{
            position: "absolute",
            left: 0,
            top: 1.473,
            width: 18,
            height: 16.996,
            overflow: "hidden",
          }}>
            <svg width={18} height={13} viewBox="0 0 18 13" fill="none" style={{
              position: "absolute",
              left: 0,
              top: 3.996,
              width: 18,
              height: 13,
              color: "rgb(255,255,255)",
            }}>
              <path d={"M 0 2 L -1 2 L 0 2 Z M 2 0 L 2 -1 L 2 0 Z M 16 0 L 16 -1 L 16 0 Z M 0 11 L -1 11 L 0 11 Z M 0 2 L 1 2 C 1 1.735 1.105 1.48 1.293 1.293 L 0.586 0.586 L -0.121 -0.121 C -0.684 0.441 -1 1.204 -1 2 L 0 2 Z M 0.586 0.586 L 1.293 1.293 C 1.48 1.105 1.735 1 2 1 L 2 0 L 2 -1 C 1.204 -1 0.441 -0.684 -0.121 -0.121 L 0.586 0.586 Z M 2 0 L 2 1 L 16 1 L 16 0 L 16 -1 L 2 -1 L 2 0 Z M 16 0 L 16 1 C 16.265 1 16.52 1.105 16.707 1.293 L 17.414 0.586 L 18.121 -0.121 C 17.559 -0.684 16.796 -1 16 -1 L 16 0 Z M 17.414 0.586 L 16.707 1.293 C 16.895 1.48 17 1.735 17 2 L 18 2 L 19 2 C 19 1.204 18.684 0.441 18.121 -0.121 L 17.414 0.586 Z M 18 2 L 17 2 L 17 11 L 18 11 L 19 11 L 19 2 L 18 2 Z M 18 11 L 17 11 C 17 11.265 16.895 11.52 16.707 11.707 L 17.414 12.414 L 18.121 13.121 C 18.684 12.559 19 11.796 19 11 L 18 11 Z M 17.414 12.414 L 16.707 11.707 C 16.52 11.895 16.265 12 16 12 L 16 13 L 16 14 C 16.796 14 17.559 13.684 18.121 13.121 L 17.414 12.414 Z M 16 13 L 16 12 L 2 12 L 2 13 L 2 14 L 16 14 L 16 13 Z M 2 13 L 2 12 C 1.735 12 1.48 11.895 1.293 11.707 L 0.586 12.414 L -0.121 13.121 C 0.441 13.684 1.204 14 2 14 L 2 13 Z M 0.586 12.414 L 1.293 11.707 C 1.105 11.52 1 11.265 1 11 L 0 11 L -1 11 C -1 11.796 -0.684 12.559 -0.121 13.121 L 0.586 12.414 Z M 0 11 L 1 11 L 1 2 L 0 2 L -1 2 L -1 11 L 0 11 Z"} fill="currentColor" fillRule="nonzero" />
            </svg>
            <svg width={8} height={4} viewBox="0 0 8 4" fill="none" style={{
              position: "absolute",
              left: 5.001,
              top: 0,
              width: 8,
              height: 4,
              color: "rgb(255,255,255)",
            }}>
              <path d={"M -1 4 C -1 4.552 -0.552 5 0 5 C 0.552 5 1 4.552 1 4 L 0 4 L -1 4 Z M 2 0 L 2 -1 L 2 0 Z M 6 0 L 6 -1 L 6 0 Z M 8 2 L 7 2 L 8 2 Z M 7 4 C 7 4.552 7.448 5 8 5 C 8.552 5 9 4.552 9 4 L 8 4 L 7 4 Z M 0 4 L 1 4 L 1 2 L 0 2 L -1 2 L -1 4 L 0 4 Z M 0 2 L 1 2 C 1 1.735 1.105 1.48 1.293 1.293 L 0.586 0.586 L -0.121 -0.121 C -0.684 0.441 -1 1.204 -1 2 L 0 2 Z M 0.586 0.586 L 1.293 1.293 C 1.48 1.105 1.735 1 2 1 L 2 0 L 2 -1 C 1.204 -1 0.441 -0.684 -0.121 -0.121 L 0.586 0.586 Z M 2 0 L 2 1 L 6 1 L 6 0 L 6 -1 L 2 -1 L 2 0 Z M 6 0 L 6 1 C 6.265 1 6.52 1.105 6.707 1.293 L 7.414 0.586 L 8.121 -0.121 C 7.559 -0.684 6.796 -1 6 -1 L 6 0 Z M 7.414 0.586 L 6.707 1.293 C 6.895 1.48 7 1.735 7 2 L 8 2 L 9 2 C 9 1.204 8.684 0.441 8.121 -0.121 L 7.414 0.586 Z M 8 2 L 7 2 L 7 4 L 8 4 L 9 4 L 9 2 L 8 2 Z"} fill="currentColor" fillRule="nonzero" />
            </svg>
          </div>
        </div>
        <div style={{
          position: "absolute",
          left: 248.751,
          top: 14,
          width: 162.982,
          height: 21,
          overflow: "hidden",
        }}>
          <span style={{
            position: "absolute",
            left: 24.982,
            top: 0,
            width: 138,
            height: 21,
            fontFamily: "Onest, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif",
            fontWeight: 500,
            fontSize: 16.63400650024414,
            whiteSpace: "nowrap",
            lineHeight: "100%",
            color: "rgb(131,131,131)",
          }}>Personal Settings</span>
          <div style={{
            position: "absolute",
            left: 0,
            top: 0.906,
            width: 18.315,
            height: 18.315,
            overflow: "hidden",
          }}>
            <svg width={7.044} height={7.044} viewBox="0 0 7.044 7.044" fill="none" style={{
              position: "absolute",
              left: 5.636,
              top: 3.52,
              width: 7.044,
              height: 7.044,
              color: "rgb(131,131,131)",
            }}>
              <path d={"M 3.522 7.044 L 3.522 7.749 C 5.857 7.749 7.749 5.857 7.749 3.522 L 7.044 3.522 L 6.34 3.522 C 6.34 5.078 5.078 6.34 3.522 6.34 L 3.522 7.044 Z M 7.044 3.522 L 7.749 3.522 C 7.749 1.188 5.857 -0.704 3.522 -0.704 L 3.522 0 L 3.522 0.704 C 5.078 0.704 6.34 1.966 6.34 3.522 L 7.044 3.522 Z M 3.522 0 L 3.522 -0.704 C 1.188 -0.704 -0.704 1.188 -0.704 3.522 L 0 3.522 L 0.704 3.522 C 0.704 1.966 1.966 0.704 3.522 0.704 L 3.522 0 Z M 0 3.522 L -0.704 3.522 C -0.704 5.857 1.188 7.749 3.522 7.749 L 3.522 7.044 L 3.522 6.34 C 1.966 6.34 0.704 5.078 0.704 3.522 L 0 3.522 Z"} fill="currentColor" fillRule="nonzero" />
            </svg>
            <svg width={12.032} height={3.379} viewBox="0 0 12.032 3.379" fill="none" style={{
              position: "absolute",
              left: 3.141,
              top: 12.68,
              width: 12.032,
              height: 3.379,
              color: "rgb(131,131,131)",
            }}>
              <path d={"M -0.602 3.013 C -0.804 3.345 -0.699 3.779 -0.366 3.981 C -0.034 4.183 0.399 4.078 0.602 3.746 L 0 3.379 L -0.602 3.013 Z M 2.566 0.903 L 2.911 1.517 L 2.911 1.517 L 2.566 0.903 Z M 9.466 0.903 L 9.121 1.517 L 9.121 1.517 L 9.466 0.903 Z M 11.43 3.746 C 11.633 4.078 12.066 4.183 12.398 3.981 C 12.731 3.779 12.836 3.345 12.633 3.013 L 12.032 3.379 L 11.43 3.746 Z M 0 3.379 L 0.602 3.746 C 1.167 2.817 1.963 2.049 2.911 1.517 L 2.566 0.903 L 2.221 0.289 C 1.062 0.939 0.09 1.878 -0.602 3.013 L 0 3.379 Z M 2.566 0.903 L 2.911 1.517 C 3.859 0.984 4.928 0.704 6.016 0.704 L 6.016 0 L 6.016 -0.704 C 4.687 -0.704 3.38 -0.362 2.221 0.289 L 2.566 0.903 Z M 6.016 0 L 6.016 0.704 C 7.103 0.704 8.173 0.984 9.121 1.517 L 9.466 0.903 L 9.811 0.289 C 8.652 -0.362 7.345 -0.704 6.016 -0.704 L 6.016 0 Z M 9.466 0.903 L 9.121 1.517 C 10.069 2.049 10.864 2.817 11.43 3.746 L 12.032 3.379 L 12.633 3.013 C 11.942 1.878 10.97 0.939 9.811 0.289 L 9.466 0.903 Z"} fill="currentColor" fillRule="nonzero" />
            </svg>
            <svg width={18.315} height={18.315} viewBox="0 0 18.315 18.315" fill="none" style={{
              position: "absolute",
              left: 0,
              top: 0,
              width: 18.315,
              height: 18.315,
              color: "rgb(131,131,131)",
            }}>
              <path d={"M 9.158 18.315 L 9.158 19.02 C 14.605 19.02 19.02 14.605 19.02 9.158 L 18.315 9.158 L 17.611 9.158 C 17.611 13.826 13.826 17.611 9.158 17.611 L 9.158 18.315 Z M 18.315 9.158 L 19.02 9.158 C 19.02 3.711 14.605 -0.704 9.158 -0.704 L 9.158 0 L 9.158 0.704 C 13.826 0.704 17.611 4.489 17.611 9.158 L 18.315 9.158 Z M 9.158 0 L 9.158 -0.704 C 3.711 -0.704 -0.704 3.711 -0.704 9.158 L 0 9.158 L 0.704 9.158 C 0.704 4.489 4.489 0.704 9.158 0.704 L 9.158 0 Z M 0 9.158 L -0.704 9.158 C -0.704 14.605 3.711 19.02 9.158 19.02 L 9.158 18.315 L 9.158 17.611 C 4.489 17.611 0.704 13.826 0.704 9.158 L 0 9.158 Z"} fill="currentColor" fillRule="nonzero" />
            </svg>
          </div>
        </div>
      </div>
    </div>
  );
}
export default AccountSettingsWorkspaceSettings10;
