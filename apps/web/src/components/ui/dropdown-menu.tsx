...
/** Prototype menu: 14px radius, 8px padding, the `ss-menu-in` entrance. */
...
>(({ className, sideOffset = 8, ...props }, ref) => (
  <DropdownMenuPrimitive.Portal>
    <DropdownMenuPrimitive.Content
      ...
      className={cn(
        'z-50 min-w-[12rem] overflow-hidden rounded-[14px] bg-surface p-2 shadow-menu',
...
function DropdownMenuCheck({ checked }: { checked: boolean }) {
  return checked ? <Check className="ml-auto h-[18px] w-[18px] text-ink" aria-hidden /> : null;
}
